...
- [ ] **Step 4: הרץ את הבדיקות ואת שומר הטהרה**
...
- [ ] **Step 5: מוטציה שמוכיחה שהבדיקה המרכזית חיה**
...
- [ ] **Step 2: הרץ וּודא כישלון (`ENOENT`)**
...
- [ ] **Step 4: עדכן את החוזה, מיד אחרי הסעיף של Task 3**
...
- [ ] **Step 1: העבר את הבורר לקובץ משלו (שינוי שם, ⛔ לא כתיבה מחדש)**
...
- [ ] **Step 2: הרץ את הסוויטה כדי לראות בדיוק מה נשבר**
...
- [ ] **Step 3: כתוב את בדיקת המסך החדש**
...
- [ ] **Step 4: הרץ וּודא כישלון (`ENOENT`)**

```bash
npx vitest run components/LevelMapScreen.test.ts
```

- [ ] **Step 5: כתוב את הרכיב**
...
- [ ] **Step 7: הרחב את `EXPECTED_CONSOLE` בהארנס**
...
- [ ] **Step 8: הרץ את ארבע הפקודות + הארנס**
...
- [ ] **Step 9: מדוד את המספר הגדול בצפייה הראשונה (D-028)**
...
- [ ] **Step 10: הרץ את בדיקת הצבע של `dataviz`**
...
- [ ] `plan/50-tasks.md`: T-079 · T-080 · T-081 ⬜ → 🟣 (בביקורת), עם המחזור והקבצים בפועל.
- [ ] `plan/30-architecture.md`: שתי שורות חוב שנולדו כאן — ⓐ `unseen` נגזר בחיסור כי אין anti-join מ-PostgREST · ⓑ טקסט המיגרציה נבדק, ⛔ הרצתה לא (אותו פער של `authAttemptsSchema.test.ts`).
- [ ] `plan/00-control.md`: `CYCLE_ID` · `ACTIVE_TASK_ID` · `NEXT_AGENT=CRITIC` · שחרור נעילה · `MILESTONE_TICKS` +1 · שורה ביומן 0.1.
- [ ] **פריט ל-PM בדיווח** (⛔ לא לקובץ של ה-PM): T-084 דורשת ספירות לשש הרמות, ו-T-080 מקבע תשובה לרמה אחת — נדרשת הכרעת מפרט (שדה `levels[]` או נתיב שני).
- [ ] `npm run measure:plan` לפני הקומיט האחרון (C-0167 נמדד: דילוג עליו הפיל את `plan-tables` באדום בטיק הבא).
