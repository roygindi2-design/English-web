...
## משימה 6 — `/study` מתחבר לתור (T-065, חלק ב׳) — ✅ **בוצעה C-0102**

**קבצים:** `app/study/page.tsx` · `components/StudyDeckScreen.tsx` (חדש)

- [x] `app/study/page.tsx` נשאר Server Component: קורא `searchParams.deck`, מאמת ב-`parseDeckName`, ומרנדר `<StudyDeckScreen deck={deck} />`. ⛔ בלי `fetch` בשרת.
- [x] `StudyDeckScreen` ('use client') מושך `apiGet<QueueResponse>('/api/study/queue?deck=…')` דרך `lib/api/client.ts` — ⛔ אין `fetch` ישיר ברכיב.
- [x] חמישה מצבים, כולם קיימים בקוד ולא רק בתוכנית:
  ...
  · **ריק** — `StudyEmptyState` הקיים + פעולה אחת
  · **כרטיסים** — `<CardDeck>`
- [x] `onGraded`: `deck='due'` ⇒ `apiPost('/api/review', …)` · `deck='unknown'` ⇒ `apiPost('/api/practice', {word_id, grade})`. הבדיקה מודדת **הכלה בסוגריים** של ענף `unknown`, ⛔ לא מרחק תווים, ואומתה במוטציה (החלפת שני המסלולים ⇒ אדום).
- [x] `elapsed_ms`: `Math.min(Math.round(…), MAX_ELAPSED_MS)`, המספר **מיובא** מ-`lib/core/reviewRequest.ts` ⛔ ולא נכתב מחדש כ-600000.
- [x] כשל בסימון ⇒ הודעת אופליין/שמירה **ו-`throw`** — הזריקה היא מה שמשאיר את הכרטיס בחפיסה.

⚠️ **ארבע סטיות מדווחות (C-0102), כולן נמדדו ולא שוערו:**

ⓐ **`apiGet` שונה ב-`lib/api/client.ts`.** הוא זרק `Error` על כל תשובה שאינה 2xx וזרק איתה
את גוף התשובה — כלומר את **הקוד**, שהוא כל מה שמבדיל בין `session_expired` (התחבר שוב),
`schema_missing` (המאגר לא הוקם) ו-`unavailable` (נסה שוב). מסך שאינו יכול להבדיל ביניהם
מציג משפט אחד לשלושתם, ושניים מהשלושה יהיו שקר. התיקון מיישר אותו עם `apiPost` ועם החוזה
שהקובץ עצמו מצהיר. ⚠️ **נמדד לפני השינוי: אפס קוראים** ל-`apiGet` ב-`app/` וב-`components/`.

ⓑ **ה-`<ActionBar>` של `/study` עבר לרכיב הלקוח, ובדיקת F-027 בקובץ `ActionBar.test.ts`
הופנתה אליו.** הסרגל שייך למצב, ורק רכיב הלקוח יודע באיזה מצב הלומד נמצא. השארת שם הקובץ
הישן הייתה מפילה בדיקה על קוד תקין; מחיקת השורה הייתה מוציאה את `/study` משומר F-027 —
בדיוק הכשל שהשומר קיים בשבילו. ⛔ **במצב הכרטיסים אין סרגל בכוונה:** הוא `fixed` לתחתית
והיה יושב על שני כפתורי הסימון, שהם עצמם הדרך קדימה במצב הזה.

ⓒ **הפעולה במצב הריק תלוית-חפיסה.** «אין מה לחזור היום — התחל מילים חדשות» היא הדרך החוצה
מ-`due` ריקה; על חפיסת `unknown` אותה פעולה היא קישור למסך שהלומד כבר עומד בו, כלומר בדיוק
המבוי הסתום של F-027. שם החפיסה השנייה משתמש בדרך החוצה **הקיימת** של החפיסה (`/cards`),
⛔ ולא בניסוח חדש.

ⓓ **`check:mobile` נפל ותוקן בהארנס, אחרי שנמדד שהכשל אינו במסך.** ההארנס מריץ `next start`
בלי ENV של Supabase, ולכן `/api/study/queue` עונה 503 **לפי החוזה שלו**, ו-Chromium רושם כל
משאב שאינו 2xx כשגיאת קונסולה. ⛔ פטור גורף ל-`/study` היה מסתיר גם חריגה אמיתית, ולכן
נוסף `EXPECTED_CONSOLE` הקשור ל**כתובת אחת ולסטטוס אחד** — 401 או 500 מאותו endpoint עדיין
מפילים. `m.location().url` נוסף לשורת השגיאה כי טקסט ההודעה של Chromium אינו נוקב במשאב.
**אחרי התיקון: 609/609 ✅ בשלושת הרוחבים.**

⚠️ **חוב טכני חדש TD-28:** `elapsed_ms` נמדד מהסימון הקודם (ומטעינת התור, לכרטיס הראשון)
ו⛔ לא מרגע שהכרטיס נכנס למסך — `<CardDeck>` אינו מדווח את הרגע הזה היום.

## משימה 7 — הבורר בלשונית `כרטיסיות` (T-065, חלק ג׳) — ✅ **בוצעה C-0103**

**קבצים:** `components/CardsScreen.tsx` (+`.test.ts` אם נוצר) · `app/dev/tabs/cards/page.tsx`

- [x] `CardsScreen` הופך ל-'use client' ומושך את שני המונים (`?deck=due&limit=1` ו-`?deck=unknown&limit=1`) — שתי הכתובות **כתובות במלואן** בקוד ולא מורכבות מתבנית, ובדיקה דורשת אותן כמחרוזות. `total` הוא הנקרא, ו-`cards.length` **נאסר בבדיקה**: הוא היה מדפיס «1» לכל חפיסה לא-ריקה — מספר שתמיד שגוי ולעולם לא שגוי בעליל.
- [x] שלושה כרטיסי כניסה, **תמיד שלושה**. הבדיקה סופרת `key: '` בדיוק שלוש פעמים, ולכן כרטיס רביעי או חסר מפילים אותה.
- [x] חפיסה ריקה = **מושבתת עם המספר**. ממומש ב-**גוף אחד משותף** (`const body = (`) שמופיע בשני הענפים — הבדיקה סופרת `{body}` פעמיים בתוך אזור-הסוגריים של `{entry.enabled ? (`, ולכן ענף שיאבד את המספר אינו יכול להישאר ירוק. **מוטציה אימתה** (הגוף הוחלף בתווית בלבד בענף המושבת ⇒ אדום).
- [x] «משפטים · נעול» — `href: null` **בתוך אובייקט הכניסה**, נמדד בהכלת סוגריים ולא במרחק תווים, בתוספת שלילת `/study` ו-`/sentences` באותו אזור. `enabled: true` נושא `href: string` **בטיפוס עצמו** (איחוד מובחן), ולכן ענף הניווט אינו יכול לקבל כרטיס בלי יעד — הנעילה נאכפת בקומפיילר. **מוטציה אימתה** (מתן יעד ל-`sentences` ⇒ אדום).
- [x] ⛔ **בלי `<ActionBar>`** — נאכף בבדיקת מקור (`ActionBar` ו-`data-action-bar` אסורים בקובץ) **וגם** בהארנס (`no action bar on a tab screen`).
- [x] כשל fetch ⇒ שלושת הכרטיסים מושבתים עם «—», ⛔ לא מסך שגיאה ו⛔ לא ריק. נאכף בשלילה: הקובץ אינו מכיל `FAILURE_HE` ואינו מכיל `StudyEmptyState`. **זה בדיוק מה שההארנס מדד** — הפיקסצ'ר בלי ENV מקבל 503 לשני המונים ונוחת במצב «—».

⚠️ **סטייה אחת מדווחת: `scripts/verify-mobile.mjs` נגעה, למרות שאינה ברשימת הקבצים של המשימה.** ברגע ש-`<CardsScreen>` מבצע רשת, `/dev/tabs/cards` צבר שתי שגיאות קונסולה של 503 ו-`clean console` נפל בשלושת הרוחבים. אותו נימוק ואותה צרות כמו ב-C-0102: הכשל אינו במסך אלא בהארנס שמריץ `next start` בלי ENV, ולכן נוסף `EXPECTED_CONSOLE['/dev/tabs/cards']` **הקשור לשתי הכתובות המדויקות ולסטטוס אחד** — 401 או 500 מאותו endpoint עדיין מפילים. ⛔ לא פטור לנתיב ו⛔ לא הורדת הפיקסצ'ר מהרשימה.

⚠️ **`<StudyEmptyState>` נותר עם קורא יחיד (`<StudyDeckScreen>`)** — «אין כרטיסיות» בלשונית היה טענה על המוצר כולו שנאמרת ממדידה של אפס חפיסות. התיעוד בראש הקומפוננטה עודכן כדי שהקורא הבא לא יחזיר אותו לכאן בטעות.
