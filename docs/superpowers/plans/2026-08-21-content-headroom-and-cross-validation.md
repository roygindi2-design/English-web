...
Expected — ⛔ **נמדד 2026-08-21, ⛔ ואינו ניחוש:** `gold` נושא **3,301** שורות `!` על **2,929** למות; H3 ו-H4 ⛔ אינם קיימים בריפו ⇒ הסעיף חייב להדפיס

> ⚠️ **תוקן C-0259 מ-3301 ל-3291 — נמדד ⛔ ולא שוער (F-113).** ‏`3301` הוא `gold.lowGlosses`,
> כלומר **ספירת שורות**; `tallyCrossValidation` עובר על `entry.low` שהיא **קבוצה**, ולכן סופר
> זוגות `(lemma, גלוסה מנורמלת)` **נבדלים**. ההפרש נמדד ישירות: `lowGlosses 3301` · `distinct 3291`
> ⇒ **10** שורות ב-H1 חוזרות על גלוסה שכבר נישאת לאותה למה. ⛔ הקוד ⛔ אינו משתנה — אותה מחרוזת
> עברית לאותה למה היא **טענה אחת** לאימות, ⛔ ולא שתיים; המספר בתוכנית הוא שהיה שגוי.
