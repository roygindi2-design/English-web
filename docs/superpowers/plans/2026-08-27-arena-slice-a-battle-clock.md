...
- [ ] **צעד 1 — T-173 · הבדיקה האדומה.** צור `lib/core/arenaWords.test.ts` **לפני** המימוש. הבדיקות, כלשונן:
...
- [ ] **צעד 2 — T-173 · המימוש.** צור `lib/core/arenaWords.ts` עם החתימה מ-§ 5. ⛔ אפס import מ-`lib/api`, ⛔ אפס `fetch`, ⛔ אפס React.
- [ ] **צעד 3 — T-173 · ירוק.** `npx vitest run lib/core/arenaWords.test.ts` ⇒ ירוק.

### T-176 — ליבת הקרב (טהורה · 90 שניות)

- [ ] **צעד 4 — T-176 · הבדיקה האדומה.** צור `lib/core/battle.test.ts` **לפני** המימוש:
...
- [ ] **צעד 5 — T-176 · המימוש.** צור `lib/core/battle.ts` לפי § 5.
- [ ] **צעד 6 — T-176 · ירוק.** `npx vitest run lib/core/battle.test.ts` ⇒ ירוק.
...
- [ ] **צעד 7 — T-177 · הטוקנים.** צור `app/arcade/arcade-tokens.css` עם **חמשת** הערכים של אינווריאנט
`37 § 13.5` בלבד: `#d4a94a` · `#f5d684` · `#4a4858` · `#34323f` · `#1c2642`.
⛔ **אף אחד מהם ⛔ אינו נכנס ל-`lib/core/palette.ts`** — הבדיקה בצעד 8 אוכפת זאת.

- [ ] **צעד 8 — T-177 · הבמה.** ערוך `app/arcade/page.tsx` · `app/dev/arcade/page.tsx` · `components/ArenaStage.tsx`
...
- [ ] **צעד 9 — הפרישה. ⛔ ⛔ לא «אחר כך».** מחק `lib/core/arcadeBattle.ts` · `lib/core/arcadeBattle.test.ts` · `components/ArenaBoard.tsx`
...
- [ ] **צעד 10 — הליכת ראייה.** `npx next dev -p 3000`, ואז `/dev/arcade` ב-**375×780** (`app/dev/arcade/page.tsx`).
...
- [ ] **צעד 11 — צעד הסיום (§ A1 שורה 3).** `npm run verify` ⇒ ירוק, **בהרצה טרייה**.
