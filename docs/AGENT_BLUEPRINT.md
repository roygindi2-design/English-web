...
**המכסה — מקור יחיד: `plan/RULES.md § 0.1.1 א׳` (11/08/2026).** התקציב הוא **קרדיטים**,
...
שגוי ובטל לפי § 0.1.1 א׳. חריגה משעה את האתר עד סוף החודש הקלנדרי.
קדם אך ורק כשמתקיימים כל התנאים:
  ...
  3. **עד קידום אחד ליממה** — `RULES § 0.1.1 ב׳`, שביטל במפורש גם את "פעם ב-20 שעות"
...
⛔ **אין `[skip ci]` באף קומיט — התג בוטל 2026-08-06 (`RULES § 0.7`).** הוא ישב על ראש
`main` אחרי `--ff-only` וגרם לקידום מוצלח שלא העלה דבר לאוויר. הודעות הקומיט נקיות.
**חובה אחרי כל קידום: בדיקת עשן ל-`/api/health` באתר החי — `"ok": true`** (`§ 0.1.1 ד׳`).
...
⚠️ **המספרים כאן כפופים ל-`plan/RULES.md § 0.1.1 א׳` (11/08/2026), שהוא המקור היחיד.**
...
* **עד קידום אחד ליממה** (`RULES § 0.1.1 ב׳` — ביטל את "פעם ב-20 שעות" ואת "כל 6 שעות";
  Critic שסקר נקי ולא קידם היום — מקדם, וקידם כבר היום — לא שוב), **וגם**
* אחרי הקידום נמשך `/api/health` מהאתר החי ומחזיר `"ok": true` (`§ 0.1.1 ד׳`).
...
הפירוט המלא: `plan/RULES.md` § 0.9 ו-§ 0.10. התוכנית: `docs/superpowers/plans/2026-08-06-content-bank.md`.
