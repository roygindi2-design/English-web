...
🔴 **⟦הוסרה 17/09 · `C-0692`⟧ טבלת המחברים לכל סוכן שעמדה כאן — ⛔ נמחקה, ⛔ ולא עודכנה.**

🔬 **הסיבה נמדדה, ⛔ ולא שוערה.** הטבלה נקראה מ-`mcp_connections` ב-14/09 ונכתבה כאן כאמת.
נמדד מולה **17/09**: ‏PM איבד `Descript` וקיבל `Kernel` · DEV קיבל `Three-js` · PROMOTER
קיבל **`Mem0`** — בעוד השורה האחרונה באותה טבלה הכריזה «⛔ ומה ש⛔ אין לאיש: … `Mem0`».
⇒ **סוכן קרא כאן טענה שגויה במסמך המוסמך שלו עצמו, שלושה ימים אחרי שנכתבה.** זהו `F-268`
בדיוק — אותה תקלה שהקפיאה את PROMOTER — קובץ אחד הלאה.

⇒ **⛔ אין כאן טבלה חדשה במקומה, ובכוונה.** כל טבלה כזאת תתיישן שוב.
**הכלל שמחליף אותה, והוא ב-`docs/agents/TOOLS.md § 0ⓐ`:**
...
📎 **וחוזה השימוש בכל כלי — מתי · איזו קריאה · איזו ראיה חייבת לחזור · מה זה ⛔ אינו מוכיח
· ניקוי — חי ב-`docs/agents/TOOLS.md`**, ⛔ ולא כאן. הקובץ הזה חוזר להיות מה שהוא: אינדקס סקילים.
