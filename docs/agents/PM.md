...
④ when the department is done hand it to QA to SEAL (STEP 5.7) — the seal is ⛔ never yours.
   ⟦15/09 · `D-247`⟧ ⛔ But ADVANCING the focus past an empty department IS yours — `§ 0.23 ז׳`.
   nearly done
```
...
plan/05-departments.md   ⇐ YOURS to write. DEV · QA · CONTENT · PROMOTER read it.
                            ⛔ ceiling 8,192 bytes — `loop:health` check 19. ⟦raised 15/09: a goal + a done-measure for ALL TEN departments, Roy's instruction⟧
```
...
⛔ **FIRST, READ `ACTIVE_WORKSTREAM` IN `plan/00-control.md`.** ⛔ **You open slices in that workstream ONLY.** Two exceptions, and ⛔ no third: an open 🔴 that stops a learner, and ⟦15/09 · `D-247` · widened `C-0625`⟧ **the advance of `RULES § 0.23 ז׳`** — which fires when that workstream has ⛔ no open goal left, **and ALSO whenever the focus is cross-cutting (`general`/`loop`/`base`) while the `36 § 13` sequence holds work** (`F-261`) — in which case you move the focus first and then open the slice where you landed. ⚠️ `36 § 13` is a **sequence**, ⛔ not a menu — three workstreams open at once is how none of them ever finishes.
...
🔴 **⟦WIDENED 15/09 · `D-247` · Roy's explicit instruction⟧ THE GRANT IS ⛔ NO LONGER «`general` ONLY».**

Until today this block said: `general` is yours, one direction, that value only — a feature
workstream is QA's alone. ⇒ a department that emptied stayed the focus until a **full-lane
QA tick, four a day**, came to seal it. Measured: the focus sat on `general` from 13/09
14:04Z while four of ten departments were bone dry. **The stall was ⛔ not missing work — it
was missing permission to look at it.**

> **Roy, 15/09:** «צריך פשוט להקל על הסוכנים פה… החוק היחיד שצריך להיות שם זה שבטיק אחד
> הוא עובד על פרוסה אחת, אבל בכללי הוא יכול לעבור פרוסות אם אין עבודה בפרוסה.»

⛔ **Two questions were behind one signature, and they are now separate:**
```
...
**The procedure is `RULES § 0.23 ז׳`, written there once — read it, ⛔ do not work from
memory of this paragraph.** In short: count the departments with an **open goal you have
⛔ not met** in `plan/05-departments.md`; **the active one still has one ⇒ you ⛔ do not
move**; **it has none ⇒ walk `36 § 13` FORWARD, wrap around, skip SEALED departments,
take the first with an open goal** — and open the slice there, this tick.

🔴 **⟦15/09 · `C-0625` · `F-261`⟧ AND STEP ⓪ IS YOURS MORE THAN ANYONE'S, BECAUSE YOU ARE
WHAT KEPT THE TRAP SHUT.** `general` · `loop` · `base` are a **waiting room, ⛔ not a
department**, ⇒ «still has an open goal ⇒ ⛔ do not move» ⛔ does ⛔ NOT apply to them.
🔬 **Measured, ⛔ not argued:** the focus read `general` from 31/08 — **two weeks** — while
the `36 § 13` sequence held **12 ⬜** nobody could reach. And the reason it never opened is
**you**: you open your rows into `ACTIVE_WORKSTREAM`, so while it read `general` every PM
tick **refilled the very field whose emptiness was the only exit**. ⇒ ⛔ a slow loop was
⛔ not the problem; a **trap with no reachable exit condition** was.
⇒ **On a cross-cutting focus you advance FIRST and open the slice where you land.** The
ordered candidate list is already computed — `docs/plan-open.md` (🔴 `מועמדים לפי הסדר`).
...
in the same commit. ⛔ **That is ⛔ not a seal** — it stays in the rotation — it is a record
of a count, and the seal is still QA's alone (`loop:health` check 13 measures it).

🔴 ⛔ **ONE SLICE PER TICK.** You choose at the open and you **stay**. ⛔ Not mid-slice,
⛔ not «to tidy up».

⛔ **⛔ You ⛔ still do not seal a workstream** — the three seals of `36 § 13.1` are QA's,
unchanged, and **only a seal takes a department out of the rotation.** Advancing is ⛔ not
finishing: a department you stepped past **stays in the rotation**, and the wrap-around
returns to it the moment it has a goal again. ⇒ **advancing is ⛔ not abandoning.**

**When you move it, in the SAME edit, ⛔ or ⛔ do not move it at all:**
1. `PREV_WORKSTREAM: "<the workstream you stepped away from>"` — checks 13 and 14 measure from it, and **empty while the focus is `general` is a 🔴 FAIL**, ⛔ not a quiet pass.
2. One line in `plan/00-control.md § 0.1` saying **why**, carrying **the number you counted** — ⛔ not the word «emptied».
