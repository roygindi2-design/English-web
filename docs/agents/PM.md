...
⛔ **FIRST, READ `ACTIVE_WORKSTREAM` IN `plan/00-control.md`.** ⛔ **You open slices in that workstream ONLY.** Two exceptions, and ⛔ no third: an open 🔴 that stops a learner, and ⟦15/09 · `D-247`⟧ **the advance of `RULES § 0.23 ז׳` when that workstream has ⛔ no open goal left** — in which case you move the focus first and then open the slice where you landed. ⚠️ `36 § 13` is a **sequence**, ⛔ not a menu — three workstreams open at once is how none of them ever finishes.
