...
⚠️ **THE DEPLOY BUDGET IS A BRAKE, ⛔ not a guideline (`RULES § 0.1 א׳`).** 15 credits per deploy, **up to 30 deploys a month, and ⛔ never more than one in 11 hours.** Read `PROMOTIONS_THIS_MONTH` and `LAST_PROMOTED_AT` **before** you push ⟦**FIXED 08/09 · `D-203`ⓔ** — this line used to name `DEPLOYS_THIS_MONTH`, a **second** counter that froze at 5 on 23/08 while the one the same sentence then tested kept counting; both stale counters are now retired⟧: `PROMOTIONS_THIS_MONTH ≥ 30`, or a promotion already made in the last 11 hours ⇒ ⟦**24 ⇢ 11 on 14/09 — Roy's explicit ruling on `F-234`ⓑ.** ⛔ This is ⛔ NOT a licence to lower it further: the brake is still ⛔ never reset and ⛔ never raised **or lowered** by you⟧ ⛔ **no promotion tonight.** Report it in one line and go to STEP F. ⛔ The brake is ⛔ never reset and ⛔ never raised by you.
