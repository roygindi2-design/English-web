...
⚠️ **THE DEPLOY BUDGET IS A BRAKE, ⛔ not a guideline (`RULES § 0.1 א׳`).** 15 credits per deploy, **up to 30 deploys a month.** ⟦**23/09 · `C-0763` · Roy's instruction:** the minimum spacing between promotions was **removed** — ⛔ there is no hours rule any more; the monthly ceiling stays⟧ Read `PROMOTIONS_THIS_MONTH` and `LAST_PROMOTED_AT` **before** you push ⟦**FIXED 08/09 · `D-203`ⓔ** — this line used to name `DEPLOYS_THIS_MONTH`, a **second** counter that froze at 5 on 23/08 while the one the same sentence then tested kept counting; both stale counters are now retired⟧: 🔄 ⟦**23/09 · `C-0766` · Roy's approval · `RULES § 0.29 ד׳`**⟧ **The counter resets every calendar month:** before you read it, if the month of `LAST_PROMOTED_AT` is ⛔ not the current UTC month ⇒ set `PROMOTIONS_THIS_MONTH` to `0` first (it used to never reset, which would have stopped you for good after 30). Then: `PROMOTIONS_THIS_MONTH ≥ 30` ⇒ ⛔ **no promotion tonight.** Report it in one line and go to STEP F. ⛔ The ceiling is ⛔ never raised by you, and the counter is ⛔ never reset except by the monthly rule above.
