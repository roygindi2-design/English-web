...
```
**שלך:** `find-skill` · `hebrew-content-writer` — ו-`hebrew-content-writer` הוא **חובה בכל ריצה** (`STEP 5.5`).
...
## 🚚 STEP 5.11 — האצווה שלך מגיעה למסד, ⛔ או שהיא ⛔ לא קרתה  ⟦NEW 18/09 · `C-0715` · `F-289`⟧

🔴 **קובץ JSONL ⛔ אינו מילה שהלומד רואה.** ⛔ **הטיק שלך ⛔ אינו נגמר בכתיבה לקובץ.**

🔬 **נמדד `C-0712`, ⛔ ולא שוער — וזה קרה לך:** המסד החזיק **476 מילים** בעוד האצוות
שלך החזיקו **1,483**. ‏`npm run build:ingest` קרא את כל 46 האצוות, פסל **0** שורות
וכתב את ה-SQL — ⛔ **והוא ⛔ לא הוחל מאז 16/08.** ⇒ **37 אצוות, חודש שלם של העבודה
שלך, ⛔ מעולם ⛔ לא הגיעו ללומד**, ו⛔ אף אחד ⛔ לא ידע: כל שער מודד את **הריפו**,
ו⛔ אף אחד ⛔ לא מדד את הפער בין הריפו ל**מסד**.

**שלושה צעדים, ⛔ בכל טיק שכותב אצווה:**
```
① npm run build:ingest        ⇒ האצווה נכנסת ל-supabase/seed/0001_content_batches.sql
② npm run ingest:status       ⇒ הרשימה שעל הדיסק + השאילתה להשוואה
③ mcp__Supabase__execute_sql  ⇒ select notes from generation_runs order by notes;
```
**אצווה שאינה חוזרת בשאילתה ⛔ לא הוחלה.** ⇒ קח את הבלוק שלה מ-`0001_content_batches.sql`
(‏בלוק אחד לאצווה, מתחיל ב-`-- batch-YYYY-MM-DD.jsonl`) ושלח אותו דרך `execute_sql`.

⛔ **`apply_migration` ⛔ אינו הכלי כאן** — הוא נדחה במסווג (`Reason: [Production Deploy]`,
נמדד `C-0646`). ‏`execute_sql` ⛔ **אינו** נדחה, ונמדד כותב לפרודקשן ב-`C-0712`.

🔴 **ומלכודת שעלתה ביוקר — ⛔ אל תדלג עליה.** מילה חדשה נכנסת עם
`cefr_profile_band = null`, ו-`loadNewWords` **ממיין לפי השדה הזה** ⇒ **מילה בלי פס
⛔ אינה מגיעה ללומד גם אחרי שהוחלה.** אחרי כל החלה:
```
select count(*) from words where cefr_profile_band is null;   ⇒ חייב להיות 0
```
⛔ אינו 0? הפס מגיע מ-`supabase/seed/0002_word_cefr_levels.sql` — **UPDATE בלבד**,
לפי `(headword, pos)`, אידמפוטנטי. ⇒ שלח את השורות של המילים החסרות.

⛔ **ובדוח שלך — המספר, ⛔ ולא «הוחל»:** «`words` X ⇒ Y · `senses` A ⇒ B · `null` = 0».
⛔ **«כתבתי אצווה» ⛔ אינה תפוקה. «הלומד רואה Y מילים» היא.**

## 🧰 STEP 5.10 — הכלים  ⟦NEW 17/09 · `C-0692`⟧
...
⇒ `Supabase` § 1 ⟨**קריאה בלבד** — ראה למטה⟩ · `Kernel` § 2. **ונוגעים בכל אחד רק כשהשורה אומרת.**
🔴 ⛔ **ו-`Supabase` חובר לך ב-17/09, ⛔ אבל הכלל ⛔ לא השתנה עדיין.** ‏`docs/skills-registry.md`
קבע ש⛔ אין לך מסד ⇒ «אתה כותב JSONL ו-seed, ⛔ ולא נוגע במסד». ⇒ **עד שרוי יכריע אחרת:
`list_tables` וספירה כדי לאמת שה-seed שלך נחת — ⛔ ולא `apply_migration` ו⛔ לא כתיבה.**
⛔ **ספק ⇒ שורה ב-`plan/03-for-roy.md`, ⛔ ולא כתיבה למסד.**
