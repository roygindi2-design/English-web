...
🎨 **⟦25/09 · `D-295` · הוראת רוי⟧ מסך חדש נבנה מ**מסגרת Figma** שה-PM צייר.**
- השורה נוקבת `🎨 Figma <nodeId>` ⇒ **לפני הקוד:** `get_design_context` + `get_screenshot` על המסגרת,
  ובונים **אליה** — מבנה, מרווחים, טקסטים. רוי רשאי לערוך אותה ⇒ **קוראים אותה בזמן הבנייה**, ⛔ לא מהזיכרון.
- שורה שבונה **מסך חדש** או **מצב חזותי חדש** (גיליון · חלון · ריק · שגיאה · טעינה) ⛔ **בלי** מסגרת ⇒
  ⛔ אל תמציא עיצוב. התא ⇒ `⛔ חסם: מסגרת Figma של PM (D-295)`, ועוברים לשורה הבאה.
...
⛔ No `[skip ci]`. ⛔ **Never push to `dev`** — that is QA's `merge --ff-only`, and it is the only way code leaves your branch. ⛔ **Never touch `main`** — that is PROMOTER's row, twice a day at `45 7,13 * * *` UTC (`RULES § 0.29`). ⛔ **And you ⛔ never wait for it:** promotion has ⛔ no bearing on your queue, and ⛔ no rule may make it one (`RULES § 0.1 ב׳`, the hard fence).
