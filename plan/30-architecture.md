...
| AR-3 | חוזה ה-API מתועד ב-`/docs/api-contract.md` ומעודכן בכל שינוי | ✅ | C-0003 · `/api/auth/signup`, `/api/auth/login` ו-`/logout` תועדו באותו קומיט |
| AR-4 | אין `window`/`document`/`localStorage` בתוך `/lib/core/` — קוד זה חייב לרוץ כמות שהוא ב-React Native | ✅ | C-0003 · נאכף אוטומטית; ה-session חי ב-cookies מסוג httpOnly ולא ב-`localStorage` |
...
| MF-2 | כל אזור לחיץ ≥ 44×44px | ✅ C-0003 · נמדד ב-6 מסלולים × 3 רוחבים (105 בדיקות) |
| MF-3 | תמיכת RTL מלאה לעברית + טיפול נכון בטקסט אנגלי משובץ (bidi) | 🟡 חלקי · C-0003 · שדות האימייל והסיסמה קיבלו `dir="ltr"` נקודתית בתוך פריסת RTL (הוראת תוכנית UX של T-002). זהו החוב ש-T-009 (`<EnWord>`) מחליף — ראה TD-5 |
| MF-4 | אפס גלילה אופקית בכל רוחב מסך | ✅ C-0003 · 320/375/414px, כולל מסכי האימות |
| MF-5 | פעולות ליבה בהישג אגודל (חצי מסך תחתון) | ✅ C-0003 |
