...
| TD-11 | **הכרטיסייה אינה מסמנת את מילת המטרה בתוך משפט הדוגמה.** מפרט ה-UI של T-041 קובע שהסימון הוא כל הסיבה שנשמרים שני משפטים, אך `Card` מחזיר את המשפט כמחרוזת בלבד. בכיוון `recognition` הצד האחורי כלל אינו מכיל את המילה האנגלית, ולכן ה-UI ייאלץ להצליב מול `front.primary` — וגם אז הצורה במשפט מוטה (`deliberated` מול `deliberate`) | הזיהוי דורש את `inflections()` שהוא כרגע פרטי ב-`contentSchema.ts` — קובץ שנמצא **כרגע בביקורת** (T-039 🟣); לייצא ממנו עכשיו זה לשנות קוד שה-Critic סוקר. החלופה — התאמה מחדש בשכבת React — מוציאה לוגיקה פדגוגית מ-`/lib/core` ואסורה | T-041, לפני שורת JSX ראשונה: לייצא את מנגנון הנטיות ולהחזיר `exampleTarget` מ-`buildCard`. ⚠️ **תנאי נוסף שנוסף C-0014:** `EnText` משרשר את הקטעים **מילולית ואינו מוסיף רווח ביניהם**, ולכן היצרן חייב לקיים `segments.map(s => s.text).join() === המשפט המקורי`. יצרן שמפצל במילים (`split(' ')`) או עושה `trim` ירנדר "HeranswerwasdeliberateXX" על הכרטיסייה של הלומד. החוזה כתוב על `EnTextSegment.text`; הבדיקה שאוכפת אותו חייבת להיכתב ב-`flashcard.test.ts` ברגע ש-`exampleSegments` נולד |
...
| TD-14 | **המחסום של T-009 מוכיח היעדר סימנים, לא נוכחות עטיפה.** `EnWord.test.ts` סורק מקור ולכן תופס מי שכותב `lang`/`dir`/`ltr-inline` ביד — אבל `<span>{word}</span>` בלי שום מאפיין הוא בלתי נראה לסריקת טקסט. האכיפה החיובית היא **פר־שדה**: היום `preview.headword` ו-`preview.pos` | להבחין באנגלית מעברית בביטוי JSX שרירותי דורש פרסר ועוד ניחוש שפה. סקירת סוכן משנה מצאה בפועל את `preview.pos` מרונדר עירום כשהמחסום היה ירוק — ולכן נוספה אכיפה חיובית לשני השדות האנגליים הידועים, ותועד כאן שכל שדה אנגלי חדש חייב שורה משלו | T-041 — כשמסך הכרטיסייה מוסיף שדות אנגליים, להרחיב את האכיפה החיובית באותו קומיט |
