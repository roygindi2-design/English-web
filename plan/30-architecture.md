...
| TD-13 | ⚠️ **נותר פתוח אחרי C-0033, וכעת הוא עולה בפועל — משימה 4 בתוכנית `2026-08-07-onboarding-goal.md` היא שסוגרת אותו.** C-0033 החליף את הטקסט הזמני ב-`app/onboarding/page.tsx` ב-`<OnboardingForm>` האמיתי: קבוצת רדיו, שדה תאריך ושדה ציון. **כל אלה נכתבו ונדחפו בלי שאף פיקסל שלהם נמדד** — 351 בדיקות המובייל נשארו זהות בדיוק אחרי השינוי, וזו הראיה הישירה שהמסך אינו בהארנס. **מוקף חלקית C-0018 · לא נסגר C-0032.** C-0032 הוסיף את שכבת ההתמדה (`0004_onboarding_answers.sql` + `POST /api/profile`) אך **לא נגע במסך ולא בהארנס**, ולכן `/onboarding` עדיין אינו נמדד. **מקור:** רכיב `RegisteredAddress` (T-026) נמדד דרך `app/dev/identity` בדיוק כמו פיקסטורות `/dev/card`, ולכן הוא **כן** מגובה במדידה (4 בדיקות × 3 רוחבים; מוטציה ל-`<span>` עירום הפילה 3 בדיקות מובייל + בדיקת TD-14). המסך `/onboarding` **עצמו** עדיין אינו נמדד. **`/onboarding` עדיין אינו נמדד ישירות ב-`check:mobile`.** ההארנס רץ בלי משתני סביבה של Supabase, ולכן `readSupabaseEnv()` מחזיר `null` וה-route מפנה — אומת חי ב-C-0013: `GET /onboarding` → `307` → `/login?expired=1`. כלומר כל 12 השורות שמדווחות `ok /onboarding …` מודדות בפועל את מסך ההתחברות | התיקון של F-016 בקובץ עצמו נכון בקוד ואומת בקריאה, אך **אינו מגובה במדידה** — בדיוק דפוס F-007. מסך ה-onboarding האמיתי (T-003) יכיל מוסד, ציון יעד ותאריך מבחן, ואז אי-המדידה מתחילה לעלות | T-003 — יחד עם המסך האמיתי, בסשן מדומה או במשתני סביבה של פרויקט בדיקה |
