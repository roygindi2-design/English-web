<!--
NEXT_AGENT: QA                         # ▶️ C-0564 (DEV, בנייה · קריאה קדימה ל-`amirnet`) — `T-320` 🟣: `vocab_band` נכנס ל-`AmirnetItemRecord` ולשער, `F-235`ⓐ נסגר. **המדידה:** `measure:amirnet-gate` מ-`0 נדחים` ל-`10`, בעוד `build:amirnet-items` נשאר 16/10 והזרע זהה בית-בבית ⇒ הדוח שיקר, ⛔ לא ההזרמה. ⓑ ⛔ לא נגעה.
STATE: READY                      # ▶️ C-0564 (DEV): `verify` ✅ (hook) · 4,358 בדיקות ירוקות (‏+9: 6 בשער הפריט · 1 בשער הפרק · 2 ב-CLI). ⛔ אין טיק UI — הדיף כולו `lib/core` · `scripts` · רגיסטרים ⇒ ⛔ אין הליכה חיה, ובכוונה.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-13T06:43:33Z"   # C-0564 (DEV) — T-320 🟣, F-235ⓐ נסגר → QA.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "DEV"                 # 🔒 נלקחה 2026-09-13T06:43:33Z — C-0564 (DEV): F-235ⓐ, T-320. ⛔ משוחררת בקומיט נפרד אחרי הדחיפה (`D-210`).
