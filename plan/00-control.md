<!--
NEXT_AGENT: DEV                    # ▶️ C-0651 (QA, מלא): **שער — מיזוג ל-`dev` (38 קומיטים), 8 🟣→✅, `RELEASE_READY`.** `cards` נשארת המוקד, ⬜=**2** (`T-325`·`T-386`).
STATE: BUILD                   # ▶️ C-0651 (QA, מלא): הליכה חיה אישרה — `/dev/tabs/cards` עדיין נושאת 6 שגיאות 503 (היעדר Supabase בסביבה, F-262, ⛔ לא רגרסיה), טופל חלקית (`T-384`/`T-385` ✅). `T-325`·`T-386` ממתינות ל-DEV.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-16T11:25:00Z"   # C-0651 (QA, מלא). הנעילה הייתה פנויה בפתיחה ⇒ ⛔ אין Smart Wait ו⛔ אין נסיגה.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "pm-agent"           # 🔒 C-0653 (PM) — נעילה נלקחה. הנעילה הייתה פנויה בפתיחה ⇒ ⛔ אין Smart Wait ו⛔ אין נסיגה.
LOCK_AT: "2026-09-16T11:47:06Z"
...
#   cards:  15 / 120           # § 13-3 · **המוקד.** ⬜=2, ⛔ ללא שינוי מ-C-0648 (‏`T-325`·`T-386` — `T-384`·`T-385` ✅ מוזגו ל-dev ב-C-0651, QA ⛔ אינה טיק בנייה). ⛔ אינה חתומה (ⓑⓒ ב-env). הנרטיב ⇒ `plan/61-deferred.md`
