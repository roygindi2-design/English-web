...
LOCK_HELD_BY: ""                   # שוחררה C-0495 (PROMOTER) — הטיק נעצר לפני הדחיפה ל-main, נעילה משוחררת.
LOCK_AT: "2026-09-07T23:33:00Z"
...
RELEASE_BLOCKERS: "🔴 C-0495 (PROMOTER, 07/09 23:33Z) — the runtime's own auto-mode permission classifier denied `git push origin main` (and briefly other `git log`/`git branch` calls in the same session) with 'Blocked by classifier.' The five-condition gate was fully green first: `verify` exit 0 (219 files/3612 tests/build/check:mobile 1433 checks), `loop:health` 15/17 (2 non-blocking: check 2 FAIL — dead finding F-196 in LOOP-ARCHITECTURE.md, not PROMOTER's; check 16 warn — verify-note missing on head, warning-only until 2026-09-13), `origin/main..origin/dev` = 71 commits, `merge-base --is-ancestor origin/main origin/dev` clean, deploy budget clear (14/30 promotions this month, 33h21m since last promotion). PROMOTER did not attempt to route around the block (RULES § — no destructive/force workaround is ever appropriate against a denied permission). ⛔ No merge, no push, no smoke test ran. Needs Roy: either grant PROMOTER explicit permission for `git push origin main` in this runtime, or push `beadc33..<origin/dev HEAD>` --ff-only yourself, or tell the next PROMOTER run how to proceed."  # ▶️ רוקן C-0487 (DEV) — `F-192` 🟣 (‏`D-196` ⓐ, 22/22).
DEPLOY_BRANCH: main               # Netlify בונה אך ורק את זה. 🆕 06/09: **רק PROMOTER מקדם לכאן** (`RULES § 0.29`, `21 23 * * *` UTC — ⚠️ הוזז מ-`0 23` ב-07/09 בבקשה מפורשת של רוי). רוי גובר תמיד.
