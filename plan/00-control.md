<!--
NEXT_AGENT: QA                     # ▶️ C-0690 (DEV): `T-410` 🟣 — ארבעת מסלולי `לימודים` נכנסים ב-320/375/414, גלישה 81px ⇒ 0. `verify` מלא exit 0 דרך `pre-push`. ⬜ כשירות: 12.
STATE: BUILDING                # ▶️ C-0690 (DEV): שורה אחת · `check:mobile` 2063⇒2093 (6 טענות חדשות) · הליכה חיה 320/375/414 נקייה · ⛔ `T-409` ⛔ לא נלקחה — ⛔ לא נכנסת לפני מסירת :45.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-17T11:42:00Z"   # C-0690 (DEV, בנייה). הנעילה הייתה של QA בפתיחה ⇒ **שלושה** סבבי Smart Wait, שוחררה בסבב 3.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "DEV"                    # 🔒 ננעל — C-0690 (DEV), מסלול מלא. הנעילה הייתה של QA בפתיחה ⇒ Smart Wait 3 סבבים, שוחררה בסבב 3.
LOCK_AT: "2026-09-17T11:19:40Z"
...
#   studies: 8 / 120           # § 13-5 · **המוקד.** ⬜=**1** — `T-409` בלבד (`T-410` 🟣 `C-0690`). 🟣=3 — `T-405`·`T-406` (`C-0680`) · `T-410` (`C-0690`). ✅=2 — `T-407`·`T-408` (`C-0689`). ⛔=1 (`T-144`) · `T-247` 🚫
...
WORKSTREAM_ENDING: "studies · 1 ⬜ נותרה · 2026-09-17T11:42:00Z · C-0690"   # `STEP 5.7` — `T-410` נמסרה בטיק הזה. נותרה `T-409` (התמדת מיקום: מיגרציה + `app/api/**` + רכיב) — ⛔ **⛔ לא נלקחה במכוון**: הנעילה נפתחה ב-11:19 אחרי 3 סבבי Smart Wait, ושורה בגודל 30–60 דק׳ הייתה מחזיקה את הנעילה הרבה מעבר למסירת :45. ⇒ היא הראשונה בתור לטיק הבא.
...
| C-0689 | QA | DEV | 2026-09-17T11:13:29Z | ⭐ **שער מלא — מיזוג ל-`dev` (ff-only, 23 קומיטים), 4 🟣→✅ (T-407·T-408·T-411·T-412), `RELEASE_READY`.** `verify` מלא ירוק (5006/5006·build✅·mobile 2087) · `loop:health` 23/23 · smoke 6/6 דרך Kernel · הליכה `/dev/tabs/studies`·`/dev/deck/level`·`/dev/deck` נקייה. `F-282` נפתח → DEV: מצב `level_done` (`T-412`) ⛔ אינו נגיש משום פיקסטורה, ⛔ ולא נבדק חי. `STEP 5.8`: המוקד `studies` אינו חוצה-מערכת ומחזיק ⬜=2 (`T-409`·`T-410`) ⇒ אין החלטה נדרשת. | `plan/00-control.md` · `plan/50-tasks.md` · `plan/60-findings.md` · `plan/03-for-roy.md` · `docs/plan-open.md` · `docs/plan-tables.md` |
| C-0690 | DEV | QA | 2026-09-17T11:42:00Z | 🔨 **בנייה — שורה אחת 🟣.** `T-410` — ארבעת מסלולי `לימודים` נכנסים במלואם ב-320·375·414 (‏`flex-wrap` על טיפוגרפיית הרנדר; `overflow-x-auto` ושני סימני הקצה של `T-330` הוסרו, ⛔ לא הושתקו). הליכה חיה: גלישת רצועה **81px ⇒ 0**, מסלולים גלויים **3/4 ⇒ 4/4**, שבב 44px, 0 שגיאות. `check:mobile` 2063⇒**2093**. ⛔ `T-409` ⛔ לא נלקחה — ⛔ לא נכנסת לפני מסירת :45. | `components/StudiesScreen.tsx` · `components/StudiesScreen.test.ts` · `scripts/verify-mobile.mjs` · `plan/50-tasks.md` |
