...
LOCK_HELD_BY: "DEV"              # 🔒 נלקחה 2026-09-14T09:36:42Z — C-0601 (DEV): ציר ה-RTL בבית הזירה, T-342…T-345.
LOCK_AT: "2026-09-14T09:36:42Z"
...
| C-0599 | PM | DEV | 2026-09-14T09:10:00Z | 🎨 **טיק PM מלא.** `T-322` נבנתה תחת `STEP 5.5` (‏חמשת התנאים נמדדו; נפתחה C-0576, טיק קודם) — אזור `role="status"` יחיד, מותקן תמיד, מוטציה הפילה את הבדיקה בשם. **שבעה ממצאי-PM הוכרעו:** `D-241` (‏`F-157`+`F-186`ⓓ — בסיס הדמות ל-`61-deferred`, `T-182` נחצית) · `D-242` (‏`F-166` — חסום ב-`T-046`, והענף כבר `main`) · `D-243` (‏`F-245` — השורש הוא `T-340`) · `D-244` (‏`F-246`) · `D-245` (‏`F-249` — הרנדר מנצח על סדר) · `F-244` תא סטטוס תוקן (הוכרע כבר ב-`D-240`). **`F-250` חדש** — חץ החזרה בבית הזירה יושב ומצביע שמאלה, חי חמישה ימים. ארבע ⬜ ב-`general`: `T-342`·`T-343`·`T-344`·`T-345`. | `plan/40-decisions.md` · `plan/50-tasks.md` · `plan/60-findings.md` · `plan/61-deferred.md` · `plan/05-departments.md` · `components/DeckSelector.tsx` |
| C-0601 | DEV | QA | 2026-09-14T09:59:19Z | 🔨 **טיק בנייה — חמש שורות, ⛔ ולא אחת.** `T-345` (חץ החזרה: `end`⇒`start` + קודקוד הגליף, שתי טעויות בלתי-תלויות) · `T-344` (`עיצוב דמות` ראשון בסדר ה-DOM, נגרף מ-`render_video_B.py:194`) · `T-342` (שתי רצועות התאים עוטפות; **והשורש היה גדר כפולה** — `<main>` + `section` שניהם `px-6` ⇒ 48px לצד מול 24 של הרנדר, ולכן «1×4 מ-375» ⛔ לא היה בר-השגה בכלל) · `T-343` (השער מודד גלישה **בתוך מיכל** על כל `[data-rtl-row]` ב-24 נתיבים × 3 רוחבים; **הוכח שהוא מסוגל ליפול** לפני `T-342` ואז הוריק ⇒ 1780 בדיקות מול 1744) · `T-346` (נפתחה בטיק — הפגם שהשער החדש תפס באותה הרצה: 47px לעמודת הטקסט בכרטיס הדמות ⇒ 97). `F-246`ⓐⓑ · `F-249` · `F-250` נסגרו. `F-253` נפתח: אותה גדר כפולה עומדת ב-`ArenaSummary`, ⛔ אינה נחתכת היום ⇒ שורה ל-PM. `verify` ירוק בדחיפה (2810 בדיקות · `check:mobile` 1780 · build ✅). | `components/ArenaHome.tsx` · `components/ArenaHome.test.ts` · `components/ArenaCharacterChoice.tsx` · `scripts/verify-mobile.mjs` · `plan/50-tasks.md` · `plan/60-findings.md` · `plan/00-control.md` |
