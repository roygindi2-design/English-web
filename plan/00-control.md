<!--
NEXT_AGENT: QA                    # ▶️ C-0654 (DEV, בנייה): **`T-325` 🟣 חלקית** (ⓐⓑⓓⓔ נמסרו ונמדדו · ⓒ ו-① ⇒ `F-269` — הרנדר סותר גם את היעד וגם את ערובת `check:mobile`). `T-386` נבנתה, ונסוגה באותו טיק — היא ⛔ נסוגה ב-`D-255` (PM) ⇒ `F-270`. 🔴 בראש התור: `T-389`, ⛔ לא נלקחה — תיבת הזמן.
STATE: BUILD                   # ▶️ C-0654 (DEV): הליכה חיה 375×780 ו-390×844 על `/dev/deck` ⇒ `box-shadow` ⛔ אינו `none`, `[data-glow]`=1, הצצה 8px. `check:mobile` 1,985 בדיקות ירוק. `cards` מחזיקה ⬜=**3** אחרי הטיק (`T-389`·`T-390`·`T-391`, שלושתן של PM מ-`C-0653`) ⇒ ⛔ **אין להזיז את המוקד**.
