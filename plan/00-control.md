<!--
NEXT_AGENT: CRITIC                   # ▶️ **C-0341 (DEV, 🔨 בנייה): `T-181` ⇒ 🟣 — מסך הבית של הזירה נבנה כולו (4 קומיטים · 24/24 תיבות · `verify` 2982✅ · `check:mobile` 1214✅, מ-1188). ‏`/arcade` ⛔ אינו נפתח עוד בתוך קרב.** **F-161 ✅ · F-162 ✅.** ⬜ הכשירה היחידה ב-`arena` היא `T-153` — ⛔ **של PM**.
STATE: EXECUTING                  # ▶️ 23/08 — פרוסה A (`/world/story`) ופרוסה B (תיבת הסימולציות) פתוחות. שתי תוכניות UX: **D-108 · D-108א · D-109**.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: ""                # ▶️ **C-0341: T-181 ⇒ 🟣.** ⛔ אין פעילה — ⬜ היחידה ב-`arena` היא `T-153`, **של PM**. (4 משימות · 24 צעדים · `check:plan` 9/9). הטיק הבא **בונה**, ⛔ ואינו מתכנן שוב. ⛔ **F-160 → PM** פתוח; **F-161 · F-162** נפתחו בטיק הזה.
...
LAST_HANDOFF_AT: 2026-08-28T07:12:31Z
...
LOCK_HELD_BY: DEV                 # ▶️ C-0342 — טיק תכנון על T-153 (+T-218). ⛔ אין נגיעה בקוד.
LOCK_AT: 2026-08-28T08:38:47Z
...
#   arena:  14 / 120           # § 13-4 · 🔴 **ACTIVE.** ‏**C-0341 (DEV):** `T-181` ⇒ 🟣 — מסך הבית: ליבה טהורה + `GET /api/arcade/home` (קריאה בלבד) + `ArenaHome.tsx` בקואורדינטות `render_video_B.py:107-191` + מעטפת `home|battle` ⛔ בלי ראוט חדש. **חמש סטיות מוצהרות במספר** (42⇒44px · אזור פגיעה ל-chevron · צבעי מסלול הבוס — `--success` **2.72:1**, `--brand` **3.38:1** על כחול־הליל בסכימה הבהירה ⇒ טוקני זירה · D-137 · רדיוסים 18/14 ⇒ `2xl`/`xl`). שער D-134 עבר לפר-מסך והוכח שהוא נופל («`ציוד` is 1.19:1»). **F-161 ✅ · F-162 ✅.** **⬜ כשיר: 1 — `T-153`, ⛔ של PM** (D-138: ⛔ אין מיגרציה · הזמנת תוכן). ‏(‏C-0336…C-0340 ⇐ `plan/archive/control-log.md`)
