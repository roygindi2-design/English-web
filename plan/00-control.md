<!--
NEXT_AGENT: DEV                       # ▶️ C-0541 (QA) — מוזג ל-dev. msgs⬜=T-193 (CONTENT) ⇒ DEV קדימה ל-amirnet/loop/base.
STATE: READY                      # ▶️ C-0541 (QA, מלא) — `verify` ירוק · `loop:health` 22/23 · dev מסונכרן.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-12T13:20:24Z"   # C-0541 (QA, מלא) — מיזוג ל-dev + ארכוב + מפת ארכיטקטורה.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "OPS"              # 🔒 ננעלה 2026-09-12T13:28Z — C-0542 (תפעול, בהוראת רוי): QA+PROMOTER ידני ⇒ `F-229`.
LOCK_AT: "2026-09-12T13:28:00Z"
...
| C-0538 | DEV | QA | 2026-09-12T11:54:42Z | 📝 **תוכנית + משימה 1 שלה, באותו טיק (`STEP 3`).** `2026-09-12-amirnet-simulation.md` מכסה `T-296`+`T-298`, `check:plan` 10/10. `T-296` 🟣 — **הפרק מחזיק את השעון ⇒ לזמן שנותר ⛔ אין לאן לזלוג:** `chapterStartedAtMs` הוא חותמת **הפרק הנוכחי**, והשעון הוא `chapter.seconds - (now - stamp)`. שתי מעברות, ⛔ ולא אחת: פרק שהזמן בו נגמר באמצע עובר ב-`advanceChapter()` **לפרק הבא**, ⛔ ולא לשאלה הבאה של פרק שאי אפשר לענות בו. `advance()` מקבל `(state, nowMs)` ו⛔ לא תשובה — גדר האדפטיביות, עם בדיקה על האריות. 🚶 `next start` 375×780: פרק 1 `3:59` ⇒ פרק 2 `4:00`, התקציב שלו עצמו · 0 מתחת ל-44px · 0 גלילה אופקית ב-320/375/414 · 0 שגיאות קונסול. ⚠️ **חוב מוצהר:** `simulation` נשארת «טרם» — `F-222` חוסם את הסכמה ו-`T-297` עוד ⬜ ⇒ ⛔ אין מקור לפריטים. הפירוט המלא ⇒ `plan/30-architecture.md`. | `amirnetSimulation`(+17)·`AmirnetSimulation`(+18)·`dev/amirnet/simulation`·`50-tasks`·`30-architecture`·`00-control` |
