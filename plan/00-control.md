<!--
NEXT_AGENT: QA                     # ▶️ C-0843 (DEV): `nav` ⬜=0 (יעד ① ⛔ חסום על מסגרות Figma של PM, `D-295`) ⇒ קריאה-קדימה ל-`cards` (הבאה ב-`36 § 13`): `T-502` 🟣 — «להתחיל את הרמה מחדש» היה מת לכל לומד שנכנס מ-`/cards`. `verify` ירוק ⇒ `MERGE_BLOCKERS` התיישן — של QA לנקות.
STATE: BUILDING                # ▶️ C-0843 (DEV): 4 🟣 ממתינים למיזוג (`T-499`·`T-500`·`T-501` ב-`nav` · `T-502` ב-`cards`).
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-25T14:32:18Z"   # C-0843 (DEV) ⇒ QA.
...
|---|---|---|---|---|---|
| C-0843 | DEV | QA | 14:32Z | `nav` ⬜=0 ויעד ① ⛔ חסום על מסגרות Figma (`D-295`); הרצף ⛔ אין בו שורה כשירה ל-DEV (`msgs` = `T-193` CONTENT) ⇒ המוקד ⛔ לא הוזז. קריאה-קדימה ל-`cards`: פתחתי `T-502` — הבאג החשוד מ-`02-inbox` 24/09 נמדד ותוקן: תשובת `level` נושאת `band`, הכפתור שולח אותה. ⚠️ PM: `cards` ③ («עוד 20») צריך מסגרת Figma. | `T-502` 🟣 |
