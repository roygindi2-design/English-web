<!--
NEXT_AGENT: QA                         # ▶️ C-0550 (DEV) — `T-307` 🟣 נבנתה (קריאה קדימה ל-`amirnet`; ‏`msgs` ⬜=1 והיא של CONTENT). תוכנית חדשה `2026-09-12-amirnet-levels.md` (10/10) מכסה `T-307`·`T-308`·`T-309`. ⬜ פנויות ל-DEV: `T-308`·`T-309`·`T-304`·`T-270` (amirnet) · `T-305`·`T-306` (loop) · `T-302`·`T-284` (base).
STATE: READY                      # ▶️ C-0550 (DEV): `verify` מלא ✅ (תשע פקודות) · הליכה חיה 375×780 על `/dev/amirnet/levels` — 0 מתחת ל-44px, ⛔ אפס גלילה אופקית, ⛔ אפס שגיאות קונסולה.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-12T19:49:59Z"   # C-0550 (DEV) — תוכנית `amirnet-levels` + `T-307` נחתה באותו טיק.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "content-agent"    # 🔒 ננעלה 2026-09-12T21:03:47Z — CONTENT (C-0552): הוספת explanationHe ל-K-006.
LOCK_AT: "2026-09-12T21:03:47Z"
...
#   amirnet: 12 / 120          # 41 § 8-1..3 · ▶️ C-0551 (CONTENT, **קריאה קדימה** — `msgs` נשאר ACTIVE_WORKSTREAM): `K-006` 🟣 — המאגר **כבר לא ריק**: 26 פריטי `sc`/`rs`/`rc` (רמות 1-4) נכתבו, 26/26 עברו `amirnetItemGate`/`amirnetChapterGate` האמיתיים, בקרת שלילה 0/9. ⛔ **עדיין אין קליטה ל-DB** — אין `build:amirnet-items`, זו משימת DEV, נרשם ב-`03-for-roy`. קודם ⇒ C-0550 (DEV, קריאה קדימה): `T-307` 🟣 — מסך ארבע הרמות נבנה, ו⛔ עדיין ⛔ אין ללומד דרך אליו: `T-308` (הלשונית) היא הצעד הבא. 🔴 **הפער שנמדד עדיין:** `AMIRNET_BUILT_TABS` = `[dashboard, practice]` ⇒ ⛔ אין ללומד דרך ל-`simulation` (`T-307`·`T-308`).
...
| C-0550 | DEV | QA | 2026-09-12T19:50:17Z | 🔨 **טיק תכנון + הנחתת משימה 1 באותו טיק (`STEP 3`).** ⓐ תוכנית חדשה `docs/superpowers/plans/2026-09-12-amirnet-levels.md` — `check:plan` **10/10** — מכסה `T-307`·`T-308`·`T-309`, עם טבלת פערי שכבה A נושאת את מספרי הרנדר ועם הצהרת הסטייה על מספרי הציון. ⓑ `T-307` 🟣: `components/AmirnetLevels.tsx` (+8 בדיקות) ו-`app/dev/amirnet/levels/page.tsx` עם `unlockedThrough={3}` כדי ששני המצבים יימדדו במסך אחד. ⓒ ⛔ **הלומד עדיין ⛔ אינו מגיע לשם** — `AMIRNET_BUILT_TABS` ⛔ אינו נושא `simulation`, וזו `T-308` במדויק (⛔ אינה ניתנת לפיצול: מסלול בלי לשונית הוא מסך שאין אליו דרך). ⓓ נמדד: ל-`screen_levels` ⛔ אין סטיל מיוצא ב-`docs/design/` ⇒ המקור המחייב הוא `render_video_D.py:195-235`. | `AmirnetLevels`(+בדיקה)·`app/dev/amirnet/levels`·`30-architecture`·`50-tasks`·`00-control`·`plan-open`·`architecture-map` |
