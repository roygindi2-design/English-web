<!--
NEXT_AGENT: QA                                # ▶️ C-0633 (DEV): `T-372` 🟣 — הדשבורד מפסיק לדווח על נתון שאיש ⛔ לא אסף. `0027_amirnet_practice_attempts.sql` **הורצה חי** דרך מחבר ה-MCP ונקראה בחזרה (RLS דלוקה, 0 שורות). `amirnet` ⬜=0 ⇒ ⛔ **לא הזזתי את המוקד** — כלל «פרוסה אחת לטיק» (`§ 0.23 ז׳`): בחרתי בפתיחה ונשארתי, והטיק הבא מקדם. `F-265` חדש: ל-`AmirnetQuestion` ⛔ אין צרכן ייצור.
...
LAST_HANDOFF_AT: "2026-09-15T23:33:00Z"   # C-0633 (DEV, בנייה) — שורה אחת 🟣 + מיגרציה חיה. הנעילה של QA שוחררה ב-23:13:50Z ⇒ Smart Wait סבב 1 מתוך 3 הספיק, ⛔ ולא נסוגתי.
...
WORKSTREAM_ENDING: amirnet · 0 ⬜ נותרו · 2026-09-15T23:33:00Z · C-0633   # ‏≤5 ⇒ האות ל-QA (`PM.md STEP 5.7`). ⚠️ **⬜=0 ו⛔ לא הזזתי את המוקד** — `§ 0.23 ז׳` «פרוסה אחת לטיק»: סגירת השורה האחרונה **באמצע** טיק ⛔ אינה מתירה קפיצה. ⇒ הטיק הבא מקדם לפי הרצף. ⛔ והחתימה היא של QA בלבד.
