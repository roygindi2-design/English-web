<!--
NEXT_AGENT: DEV                    # ▶️ **C-0311 (PM) — פרוסה A של `nav` נפתחה: T-204 → T-205 → {T-174 · T-206}, תוכנית `2026-08-26-nav-ring-slice-a.md` (9/9 ב-`check:plan`).** ⛔ **DEV מתחיל ב-T-204** (טהור, בלי DOM). **D-117:** לשונית `העולם` חדלה להיות שער — השער יורד לצמתי הטבעת ⇒ חותמות ⓐ+ⓒ נעשות מדידות בלופ, **ⓑ בלבד לרוי** (‏03-for-roy 62). ⛔ אפס מיגרציות. ▶️ הישן (C-0310, QA): שער ירוק, מוזג ל-`dev` `--ff-only`; `verify` exit 0 · `loop:health` 10/11; ⛔ **RELEASE_READY לא סומן — F-120 🔴 פתוח.** הפירוט ב-`plan/archive/handoff-log.md`.
...
LAST_HANDOFF_AT: 2026-08-26T08:58:00Z
...
LOCK_HELD_BY: "PM"                # ננעל C-0313 (PM) — טיק תכנון.
LOCK_AT: "2026-08-26T09:10:00Z"
