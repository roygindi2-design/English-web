...
LOCK_HELD_BY: "DEV"              # 🔒 נלקחה C-0536 (DEV) — `F-224` 🔴.
LOCK_AT: "2026-09-12T09:32:31Z"
...
| C-0536 | DEV | QA | 2026-09-12T09:44:00Z | 🔨 **טיק בנייה — `F-224` 🔴 נסגר בקוד.** כל לשונית אמירנט **בנויה** היא `<Link href>` אמיתי; היעדים במפה אחת `AMIRNET_TAB_HREF` **צמודה ל-`AMIRNET_BUILT_TABS`** — מפתח שנדלק בלי ראוט ⇒ בדיקה אדומה, ⛔ ולא 404 ללומד. לשונית שטרם נבנתה נשארת `<span aria-disabled>` עם «טרם», ⛔ ולא קישור; הלשונית הפעילה נושאת `aria-current="page"`. `PRACTICE_HREF` נטען מהמפה במקום עותק שני של אותה מחרוזת. 🚶 **הליכה חיה `next start` 375×780:** דשבורד ⇢ «תרגול» ⇒ `/world/amirnet/practice` ✅ · חזרה ⇒ `/world/amirnet` ✅ · «סימולציה» ⇒ ה-URL ⛔ לא זז (נכון). 4 מסכים: 0 גלילה אופקית · 0 מתחת ל-44px · 0 שגיאות קונסול. 🔬 **והלקח:** ארבע בדיקות הסריקה של `AmirnetTabs.test.ts` היו **ירוקות לכל אורך `F-224`** — המקור נקרא מושלם, וה-`href` שחסר הוא בדיוק מה שגְרֶפּ ⛔ אינו יכול לראות ⇒ השומר הוא **רנדר** עכשיו (`AmirnetTabs.dom.test.tsx`, 6 בדיקות). `verify` ⇒ ראה הדוח. סקילים: `superpowers:using-superpowers` · `superpowers:systematic-debugging` · `superpowers:test-driven-development`. | `AmirnetTabs`·`AmirnetTabs.dom.test`·`AmirnetDashboard`·`60-findings`·`30-architecture`·`00-control` |
