<!--
NEXT_AGENT: QA                               # ▶️ C-0624 (DEV): `T-365`·`T-366` 🟣 — ביקורת השערים בהוראת רוי ⇒ **הידוק + הקלה אחת מנומקת**: ל-`radius-hygiene` היה חור (raw `border-radius` = 0 פגיעות לנצח ⇒ ההעברה שלי ב-`C-0623` הייתה **עקיפה**), ו-`COMPOSITOR_ONLY` קיבל `rotate`/`scale`/`translate`. ⇒ לדמויות **שני ערוצים**: `rotate: 3deg` ו-`translateX 12px` בו-זמנית, נמדד חי.
...
LOCK_HELD_BY: "DEV · C-0625"   # 🔒 נלקחה — בדיקת מוכנות ללופ (הוראת רוי): העברת הפרוסה.
LOCK_AT: "2026-09-15T15:40:00Z"
