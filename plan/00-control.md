<!--
NEXT_AGENT: QA                   # ▶️ C-0707 (DEV): **`T-422` 🟣** — שני המסכים השכנים על תבנית הקרב; `summary` **84⇒0** בכל חמשת הרוחבים, `result` **305⇒80** וה-80 בודד כ-`<ActionBar>` הקבועה (**`F-284`**, נמדד גם ב-`/signup`·`/onboarding`·`/world/compose`). ‏**`T-423`ⓐ** נמסרה — מלוא הרוחב על שלושת מסכי הסקופ, `left=24⇒0` ⛔ בלי תזוזת תוכן; ⓑ/ⓒ ⛔ לא ניתנות לביצוע כלשונן ⇒ **`F-285`**, ל-PM. שער מלא exit 0.
STATE: BUILD                   # ▶️ C-0707 (DEV, בנייה): שורה אחת 🟣 (`T-422`) + `T-423`ⓐ, שני ממצאים חדשים (`F-284`·`F-285`). המוקד `arena` ⛔ לא הוזז: ⬜=9 בפתיחה ⇒ `§ 0.23 ז׳` ③.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-17T20:45:00Z"   # C-0707 (DEV, בנייה). הנעילה הייתה פנויה בפתיחה — ⛔ אפס Smart Wait.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "pm-agent"            # 🔒 C-0708 (PM). הנעילה הייתה פנויה בפתיחה (C-0707 שוחרר ב-20:45Z) — ⛔ אפס Smart Wait.
LOCK_AT: "2026-09-17T20:46:53Z"
...
#   arena:  35 / 120          # § 13-4 · **המוקד**. ⬜=**8** (`T-422` 🟣 ב-`C-0707`) · 🟣=4 · ⛔=3 · ⛔ אינה חתומה. יעד ① — נותר `T-420`ⓓ (‏`ArenaHome`, שני שורשים) ו-`F-284` שאינו של הזירה. יעד ④ — `T-423`ⓐ נמסרה, ⓑ חסומה ב-`F-285`.
...
| C-0707 | DEV | QA | 2026-09-17T20:45:00Z | 🔨 **בנייה — `T-422` 🟣, `T-423`ⓐ, ושני ממצאים שנמדדו חי.** ‏`T-422`: תבנית הגובה המדויק של הקרב הוחלה על `ArenaSummary`/`ArenaResult` עם אזור גמיש שנגלל בתוך עצמו ⇒ ‏`summary` **84⇒0** בחמישה רוחבים, `result` **305⇒80**; ה-80 בודד כ-`<ActionBar>` הקבועה ונמדד גם בשלושה מסכים שלא נגעתי בהם ⇒ `F-284`. ‏`T-423`ⓐ: מנגנון מלוא-רוחב אחד ב-`[data-arena-scope]` ⇒ `left=24⇒0` ⛔ בלי תזוזת תוכן (‏`24..351` לפני ואחרי). ⓑ ⛔ בלתי-ניתנת לביצוע כלשונה ⇒ `F-285`. | `app/arcade/arcade-tokens.css` · `components/ArenaBattle.tsx` · `components/ArenaSummary.tsx` · `components/ArenaResult.tsx` · ארבעה קובצי בדיקה · רגיסטרים |
| C-0705 | DEV | QA | 2026-09-17T19:45:00Z | 🔨 **בנייה — שורה אחת 🟣, והיא סוגרת את החצי השני של `F-277` (🔴 CRITICAL, דיווח מוצר של רוי).** `T-413`: המספר מעל החפיסה היה `totalInLevel − known − inReviewList` — כלומר **דירוג** — בעוד החפיסה עצמה ממשיכה מה**סימנייה** מאז `T-411`. ⇒ שתי אוכלוסיות … ⟨הנוסח המלא הועבר לארכיון · plan/archive/handoff-log.md · C-0705⟩ | `lib/core/levelCursor.ts` (חדש) · `lib/core/levelCursor.test.ts` (חדש) · `lib/core/levelSummary.ts` · `app/api/study/queue/route.ts` · `app/api/levels/summary/route.ts` · `components/LevelMapScreen.tsx` · `docs/api-contract.md` · רגיסטרים |
