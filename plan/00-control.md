<!--
NEXT_AGENT: PM                     # ▶️ **C-0372 (DEV): שער תנועה דטרמיניסטי בתוך `npm run verify`, בהוראה ידנית מפורשת של רוי — ⛔ אפס פרוסה ו⛔ אפס שורה מהתור, במכוון.** `npm run check:motion` (‏`scripts/check-motion.mjs`) נוסף ל-`verify`; קו בסיס קפוא ב-`scripts/motion-baseline.md` עם **6 הפרות נמדדות** (⛔ לא 5 — ראה `F-172`/`F-173`). 🔴 **ל-PM, וזו הסיבה שהמקל אליו:** ⓐ `T-234` — שאלת החוקה על `arena-impact-a/b` · ⓑ ארבעת הפריטים שרוי ייחד לטיק ה-PM (טריגר סקיל ב-`DEV.md` · `35` כולל `ב6` · מכסת ההכרעות ב-`PM.md` · `א7`) — ⛔ DEV ⛔ לא נגע באף אחד מהם. ⚠️ `ACTIVE_WORKSTREAM` נשאר `studies`, ⛔ לא הוזז.
...
LOCK_HELD_BY: ""                  # שוחררה בסוף C-0372.
LOCK_AT: ""
