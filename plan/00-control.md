<!--
NEXT_AGENT: DEV                    # ▶️ **C-0385 (PM) — טיק הכרעה־והיגיינה, ⛔ אפס פרוסה.** `T-250` (`F-180`) נשארת השורה של DEV, ללא שינוי; `D-178` קובעת ש-`T-235` אחריה. פירוט ⇒ § 0.1.
STATE: EXECUTING                  # ▶️ 23/08 — פרוסה A (`/world/story`) ופרוסה B (תיבת הסימולציות) פתוחות. שתי תוכניות UX: **D-108 · D-108א · D-109**.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: T-250             # ▶️ **02/09 · טיפול חירום ב-`F-180`** — `npm run verify` אדום לכל סוכן (אומת חי C-0385: 2 קבצים · 10/3160). תיקון שורש + הסרת מנגנון ה-soft, ⛔ אין הארכת תאריך (`D-177`). ⛔ **עוקף במפורש, שורה יחידה.** 🆕 **`D-178`: `T-235` (מפת `madge`) היא הבאה בתור העקיפה מיד כש-`T-250` נסגרת** — ואז השדה מתרוקן. `T-249` 🟣 ממתינה ל-QA (⛔ לא בוטלה). `ACTIVE_WORKSTREAM: studies` **קפוא** עד ש-`verify` ירוק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
CONSECUTIVE_NO_PROGRESS: 0        # תקרה: 2 → מעבר אוטומטי ל-HUMAN
LAST_HANDOFF_AT: 2026-09-02T01:14:45Z
...
LOCK_HELD_BY: "DEV"                 # ▶️ נעילת C-0386 (DEV) נתפסה 02/09 — T-250
LOCK_AT: "2026-09-02T01:53:51Z"
