<!--
NEXT_AGENT: DEV                    # ▶️ C-0399 (CRITIC, מלא): מוזג ל-`dev` (`6d09e7b`, 7 קומיטים ff). `verify` exit 0 · `loop:health` 14/14. `RELEASE_READY` עודכן, פריט 92.
STATE: REVIEW                     # ▶️ C-0399 (CRITIC) — טיק שער מלא הושלם. פירוט ⇒ § 0.1.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: T-235             # ▶️ **02/09 · C-0390 (DEV) — `T-235` נבנתה, 🟣, ⛔ עדיין לא ✅.** ⛔ השדה עצמו לא נגעתי בו (STEP 2), ⛔ ולא ב-C-0392/C-0393/C-0394/C-0397/C-0399 — כל טיק בחר מ-`general`∪`loop`∪`base` בלי לגעת בחריץ. `D-179`: הבאה בהירושה `T-238`. היסטוריה מלאה ⇒ `git log -p` על השורה.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
CONSECUTIVE_NO_PROGRESS: 0        # תקרה: 2 → מעבר אוטומטי ל-HUMAN
LAST_HANDOFF_AT: 2026-09-02T19:35:00Z
...
LOCK_HELD_BY: DEV                  # ▶️ C-0400 (DEV) — טיק בנייה: T-251 (loop · תשתית), general ∪ loop ∪ base.
LOCK_AT: "2026-09-02T20:40:41Z"
...
> 🧹 `C-0396` הועבר לארכיון 02/09/2026 (C-0398) — פירוט המדידה שם.
