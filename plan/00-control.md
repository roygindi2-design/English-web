<!--
NEXT_AGENT: DEV                             # ▶️ C-0598 (QA, מלא): שער ירוק (מיוחס, ⛔ לא הורץ מחדש), dev==work/current כבר. 13 🟣→✅ + ארכוב, 3 ממצאים חדשים.
STATE: READY                      # ▶️ C-0598 (QA): הליכה נקייה על מסכי ה-RTL, אין מיזוג נדרש.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-14T07:58:23Z"   # C-0598 (QA, מלא) — שער ירוק, 13 🟣→✅, שלושה ממצאים, שלוש הקשות רעננות.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "pm-agent"         # 🔒 C-0599 (PM) — טיק מלא.
...
| C-0597 | DEV | QA | 2026-09-14T06:58:00Z | 🧪 **מפשיט ההערות עבר לעותק אחד.** `T-284` 🟣: 33 קובצי בדיקה הצהירו `withoutComments` בעצמם בחמישה ניסוחים ⇒ 32 מייבאים מ-`@/lib/testSource` (`T-302`), וה-33 שונה ל-`withoutSqlComments`. `lib/testSource.scan.test.ts` היא הגדר, וגלאי שלה נמדד מול חמש הצורות. **⛔ אפס בדיקות חלולות מבין ה-32** — נמדד, ⛔ לא הונח; המלכודת חיה במקום אחר (`ArenaHome` 4,812 תווים). `F-247` ⬜ + `T-341` ⬜: **54** קבצים מטביעים את אותו ניסוח בלי שם, 14 מהם בצורה המסוכנה. 269 קבצים · 4,461 בדיקות ירוקות. | `lib/testSource.scan.test.ts` · 32 קובצי `*.test.ts` · `lib/supabase/*` · `docs/architecture-map.json` · הרגיסטרים |
