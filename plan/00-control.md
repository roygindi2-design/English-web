<!--
NEXT_AGENT: QA                     # ▶️ C-0681 (DEV, בנייה): **שורה אחת `studies` נמסרה** — `T-407` 🟣. נותרו ⬜=2 (`T-408`·`T-409`, שרשרת). ⛔ **`T-408` ⛔ לא נלקחה, ו⛔ לא בשקט:** היא דורשת חוזה חזרה **חוצה־מסכים** (`components/StudyDeckScreen.tsx` · `app/study/page.tsx`) שאינו ברשימת הקבצים שלה, ו-`/studies` סטטית בכוונה (`T-328`) ⇒ כתובת החזרה ⛔ אינה יכולה להיות פרמטר שאילתה. ⇒ השורה תת־מוגדרת, ו-PM מכריע.
...
LOCK_HELD_BY: "ops-agent"          # 🔒 C-0682 (OPS · הוראת רוי) — יעדים חדשים בכל מחלקה + ארבעה ממצאים מדודים.
LOCK_AT: "2026-09-17T07:26:00Z"
...
| C-0681 | DEV | QA | 2026-09-17T07:23:07Z | 🔨 **בנייה — שורה אחת `studies`, `T-407`.** בחירת מסלול פותחת את **נתיב המודולים** שהרנדר מצייר בחצי התחתון של `kol-A-04-learning.png`: עמוד שדרה אנכי, כרטיס מודול לכל רמה, ונקודת מצב עליו. **המודולים נגזרים** משש הרמות ש-`GET /api/levels/summary` כבר מחזיר ⇒ ⛔ אפס תוכן חדש (R-010). 🔴 **`F-276` נפתח באותו טיק:** הרנדר מצייר נעילה בין רמות ו-`R-017` אוסר אותה ⇒ המצב השלישי נמסר כ**היעדר תוכן** (רמה בלי מילים), ⛔ ולא כהיעדר רשות; ההכרעה בין שני המסמכים היא של PM. ⛔ **`T-408` ⛔ לא נלקחה** — חוזה חזרה חוצה־מסכים שאינו ברשימת הקבצים שלה, והטיק נעצר בתנאי **תיבת הזמן**. `verify` exit 0 · הליכה חיה 375×780. | `lib/core/studyTracks.ts` · `lib/core/studyTracks.test.ts` · `components/StudiesScreen.tsx` · `components/StudiesScreen.test.ts` · `scripts/verify-mobile.mjs` · `docs/architecture-map.json` · `plan/50-tasks.md` · `plan/60-findings.md` · `plan/00-control.md` |
