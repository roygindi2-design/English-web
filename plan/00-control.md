<!--
NEXT_AGENT: DEV                   # ▶️ C-0706 (PM): **שתי שורות ⬜** — `T-428` (‏`arena`, יעד ④: הכרעה ① של `36 § 8.0` חלה על **כל חמשת** מסכי הזירה, ו-`T-423` נוקבת ב-`ArenaBattle` לבד) · `T-429` (‏`loop`, **מדידה של DEV מ-`C-0704`** — `check:mobile` 281ש׳ מול ~143). ⬜ ב-`arena`: **9** (נמדד ב-`docs/plan-open.md`). ⛔ אפס בניית קוד בטיק הזה — הנימוק בדוח.
STATE: PLANNING                # ▶️ C-0706 (PM, תכנון): שתי שורות ⬜, פריט 138 לרוי, `F-265` ✅. המוקד `arena` ⛔ לא הוזז: ⬜=8 בו בפתיחה ⇒ `§ 0.23 ז׳` ③ «יותר מאפס ⇒ ⛔ לא זזים».
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-17T20:03:00Z"   # C-0706 (PM, תכנון). הנעילה הייתה פנויה בפתיחה — ⛔ אפס Smart Wait.
...
#   arena:  34 / 120          # § 13-4 · **המוקד**. ⬜=**9** (‏+`T-428`, `C-0706` — נמדד) (`T-420`·`T-421` + `T-422`…`T-427`) · 🟣=3 (`T-416` יעד ① · `T-417` יעד ② · `T-418` יעד ③, ‏`C-0704`) · ⛔=3 · ⛔ אינה חתומה. יעד ① — השארית (`ArenaSummary`/`ArenaResult`) ב-`T-422`.
