<!--
NEXT_AGENT: QA                            # ▶️ C-0594 (DEV, בנייה): 4 שורות 🟣 (T-336 · T-335 · F-243 · T-306), `verify` מלא ירוק, ⛔ אין שינוי מוצר.
...
| C-0594 | DEV | QA | 2026-09-14T02:49:00Z | 🔨 **טיק בנייה — ארבע שורות, ארבעה קומיטים, כולן על מפיקים של תוצרים נגזרים.** ‏`T-336` 🟣: `isSealedRow` התאים `/חתומה/` **בתוך** `אינה חתומה` ⇒ מחלקה שמצהירה שלא נסגרה מחקה את עצמה; ההגנה היחידה עד היום הייתה צירוף מקרים ניסוחי. קבוע `NOT_SEALED` נבדק **לפני** `OPEN_MARKS` — **6 טענות אדומות** ⇒ 15/15. ‏`T-335` 🟣: `madge` ⛔ אינו פותר `@/` בלי `--ts-config`, **ו⛔ הדגל לבדו ⛔ לא הספיק** — ל-`paths` ⛔ אין `baseUrl` ⇒ `./*` נפתר מול cwd של הפותר; `cwd` ננעץ ל-`ROOT`. נמדד: 509 רשומות · **345 ריקות (68%) ⇢ 173**, `Flashcard.tsx` `[]` ⇢ **7 קשתות**, `studyTracks.ts` 0 ⇢ **3 צרכנים**. המפה נוצרה מחדש **באותו קומיט**. ‏`F-243` 🟣: העותק הפרטי של `splitRow` ב-`archive-registers.mjs` ⛔ לא ידע על `codeSpans()` ⇒ שני הפרסרים חלקו על אינדקס התא. **ו⛔ זה גדול ממה שהממצא תיאר:** ⛔ לא `T-326` לבדה — **16 שורות משימה ו-12 שורות ממצא** (עד **20** תאים ב-`F-129`) נדחו ⛔ בשקט; `--dry` **0 ⇢ 23 שורות** (79.2KB), ⛔ אפס אזהרות, ⛔ אפס שורות פגומות. הייבוא מ-`planTable.ts` ⛔ אינו אפשרי (`.mjs` מול `.ts`) ⇒ החלופה שהממצא עצמו מנסח, ודילוג מדפיס כעת מזהה ושני מספרים. ‏`T-306` 🟣: רצף הסגירה של DEV נוקב ב-`generate-map` עם «SAME commit», מותנה ב-`app/`·`components/`·`lib/`, וטענה קוראת את **השורה** ⛔ ולא את הקובץ. 🔬 **החוט המשותף:** בארבעתם **המפיק סטה מהחוזה שהוא עצמו מצהיר עליו, וכל השערים נשארו ירוקים** — ⛔ אף אחד מהם ⛔ אינו נכשל בקול. | `scripts/archive-departments.mjs`(+test) · `scripts/generate-map.mjs`(+test) · `scripts/archive-registers.mjs`(+test) · `docs/agents/DEV.md` · `scripts/agent-prompts.test.ts` · `docs/architecture-map.json` · 4 רגיסטרים |
