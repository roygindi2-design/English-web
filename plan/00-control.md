<!--
NEXT_AGENT: DEV                    # ▶️ **C-0310 (QA) — שער ירוק, מוזג ל-`dev` `--ff-only` (5 קומיטים DEV: C-0307…C-0309).** `verify` **exit 0** (166 קבצים · 2,684 בדיקות · build ✓ · check:mobile 1,122) · `loop:health` **10/11** (11 אדום = `story` יבשה → הוזזה בטיק הזה) · הליכה חיה 375×780 על `/dev/story`(+`/done`): ⛔ אפס גלילה · ⛔ אפס יעד<44px · השאלה באנגלית עם **שלוש תשובות עבריות הומוגניות** (⛔ אין דליפת שפה כמו 23/08). F-133ⓐ + F-128 🟣→✅ (הגיעו ל-`dev`). ⛔ **RELEASE_READY לא סומן — F-120 🔴 פתוח (טוקן ב-DEV.md → רוי).** ▶️ **`ACTIVE_WORKSTREAM` הוזז `story`→`nav`; DEV לוקח מ-`nav` (4 ⬜).**
STATE: EXECUTING                  # ▶️ 23/08 — פרוסה A (`/world/story`) ופרוסה B (תיבת הסימולציות) פתוחות. שתי תוכניות UX: **D-108 · D-108א · D-109**.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: ""                # ▶️ C-0310: הזרימה הפעילה עברה ל-`nav` (4 ⬜: T-146 · T-147 · T-148 · T-174). DEV לוקח את הראשונה שם.
...
LAST_HANDOFF_AT: 2026-08-26T08:31:06Z
...
LOCK_HELD_BY: "PM"                 # ננעל C-0311 (PM) — טיק תכנון `nav`.
LOCK_AT: "2026-08-26T08:36:42Z"
