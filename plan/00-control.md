<!--
NEXT_AGENT: QA                       # ▶️ C-0538 (DEV) — `T-296` 🟣: פרק סימולציה אחד רץ עם שעון משלו. ⬜ פנויות ב-`amirnet`: `T-297` · `T-270`.
STATE: BUILDING                   # ▶️ C-0538 (DEV) — 📝 תוכנית + 🔨 משימה 1 שלה (`STEP 3`). `T-296` 🟣.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-12T11:54:11Z"   # C-0538 (DEV) — `T-296` 🟣.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: ""                 # 🔒 שוחררה 2026-09-12T11:58:58Z — C-0538 (DEV) סיים: `T-296` 🟣, תוכנית הסימולציה, `verify` מלא ירוק, נדחף.
LOCK_AT: ""
...
| C-0538 | DEV | QA | 2026-09-12T11:54:42Z | 📝 **תוכנית + משימה 1 שלה, באותו טיק (`STEP 3`).** התוכנית `docs/superpowers/plans/2026-09-12-amirnet-simulation.md` מכסה `T-296`+`T-298`, `check:plan` 10/10. `T-296` 🟣 — **הפרק מחזיק את השעון, ולכן לזמן שנותר ⛔ אין לאן לזלוג:** `chapterStartedAtMs` הוא חותמת **הפרק הנוכחי**, והשעון הוא `chapter.seconds - (now - stamp)` ⇒ פרק שנגמר עם 90 שניות מוסר לפרק הבא ⛔ כלום. שתי מעברות, ⛔ ולא אחת: פרק שהזמן בו נגמר באמצע עובר ב-`advanceChapter()` **לפרק הבא**, ⛔ ולא לשאלה הבאה של פרק שאי אפשר לענות בו. `advance()` מקבל `(state, nowMs)` ו⛔ לא תשובה — וזו גדר האדפטיביות (`41 § 8` פריט 4 · `41 § 9.2` של רוי), עם בדיקה על האריות. 🚶 **הליכה חיה `next start` 375×780:** פרק 1 `3:59` ⇒ לחיצה עד פרק 2 ⇒ `4:00`, התקציב שלו עצמו. 342 תווים · 9 מוקשים · **0 מתחת ל-44px** · **0 גלילה אופקית** ב-320/375/414 · **0 שגיאות קונסול**. 🔬 **הלקח:** `bg-brand` בתוך **הערה** שהסבירה למה אסור להשתמש בו הפיל את `palette.test.ts` — הוא קורא מקור גולמי, ו-`AmirnetDashboard.test.ts` מפשיט הערות. ⚠️ **חוב מוצהר:** הנתיב המוצרי ⛔ לא נפתח ו-`simulation` נשארת «טרם» — `F-222` חוסם את הסכמה ו-`T-297` עוד ⬜ ⇒ ⛔ אין מקור לפריטים. | `amirnetSimulation`·`amirnetSimulation.test` (17)·`AmirnetSimulation`·`AmirnetSimulation.test` (18)·`dev/amirnet/simulation`·`simulation-fixture`·`50-tasks`·`30-architecture`·`00-control` |
