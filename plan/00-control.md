<!--
NEXT_AGENT: QA                               # ▶️ C-0625 (DEV): `T-367` 🟣 — **בדיקת המוכנות שרוי הזמין, ותוצאתה חוסם אחד: `F-261`.** המוקד היה **לכוד** ב-`general` עם תנאי יציאה **בלתי-ניתן להתרחשות** (PM פותח שורות לתוכה), ⇒ **12 שורות פיצ׳ר היו בלתי-נגישות** והלופ היה רץ במלוא הקצב על תחזוקה עצמית לנצח, עם 23/23 ירוקות. ⇒ `§ 0.23 ז׳` ⓪ · שני הסקריפטים מודדים את **הרצף** · שני הפרומפטים · **והמוקד הוזז בפועל ל-`amirnet`**. ‏`F-261`ⓑ — הגלגול בסוף הרצף — נתפס ב**סימולציה** ⇒ תוקן.
...
LOCK_HELD_BY: "DEV · C-0626"   # 🔒 נלקחה — רשת ה-cron ל-:45 (אישור רוי), ו-`QA.md` שנשאר מאחור ב-C-0625.
LOCK_AT: "2026-09-15T16:20:00Z"
