<!--
NEXT_AGENT: QA                              # ▶️ C-0605 (DEV, מלא): `T-347` 🟣 — שער החריגה הלא-נתפסת ב-`verify-mobile`, מחובר לכל 9 העמודים, מוכח חי (נתיב זורק ⇒ exit 1).
STATE: READY                      # ▶️ C-0605 (DEV): שורה אחת נמסרה; הבריכה הכשירה ל-DEV (general∪loop∪base) = 8 ⬜.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-14T11:43:06Z"   # C-0605 (DEV) — T-347: `page.on('pageerror')` היה 0 מופעים מול 9 עמודים ו-44 נתיבים; עכשיו שער בלי רשימת היתר.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "DEV"                 # 🔒 נלקחה 2026-09-14T11:34:05Z — C-0605 (DEV): `T-347` — שער הקונסולה ב-`verify-mobile`.
LOCK_AT: "2026-09-14T11:34:05Z"
...
#   general: — / 120          # ⛔ מחוץ לרצף `36 § 13` ⇒ ⛔ אין לו תקרה. ▶️ C-0605 (DEV): `T-347` 🟣 — **חריגה שנזרקת בדפדפן מפילה עכשיו את `check:mobile`**, על כל נתיב · רוחב · סכמה · מסע. נמדד חי: לפני הטיק הזה נתיב שזורק קיבל `ok … clean console`.
...
| C-0605 | DEV | QA | 2026-09-14T11:43:06Z | 🔨 **טיק בנייה — `T-347`.** שער חריגה לא-נתפסת: `pageerror` היה **0 מופעים** מול **9** עמודים ו-**44** נתיבים ⇒ חריגה בדפדפן ⛔ לא נמדדה. חובר לכל 9 העמודים, ⛔ בלי רשימת היתר. הוכח חי: נתיב זורק ⇒ `check:mobile` exit **1** (320·375·414), ובאותה הרצה הבדיקה הקיימת הדפיסה עליו `clean console`. ⟨המלא: `plan/archive/handoff-log.md`⟩ | `scripts/verify-mobile.mjs` · `scripts/verify-mobile.test.ts` · הרגיסטרים |
