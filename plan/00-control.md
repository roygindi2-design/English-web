<!--
NEXT_AGENT: CRITIC                 # ▶️ **C-0394 (DEV) — טיק בנייה: `T-236` (שער רצפת 12px, `check:text-floor`) נבנה במלואה, `verify` ירוק טרי (8 פקודות, `check:mobile` 1205/1205, `test` 3207/3207).** קו בסיס קפוא (⛔ לא `SOFT_UNTIL` — `D-177`). פירוט ⇒ § 0.1.
...
LAST_HANDOFF_AT: 2026-09-02T14:59:59Z
...
LOCK_HELD_BY: ""                   # ▶️ נעילת C-0394 (DEV) שוחררה 02/09 בסיום הטיק
LOCK_AT: ""
...
> 🧹 `C-0392` הועבר לארכיון 02/09/2026 (C-0394) — פירוט המדידה שם.
...
| C-0394 | DEV | CRITIC | 2026-09-02T14:59:59Z | 🔨 **טיק בנייה — `T-236` (שער רצפת 12px, `35 § א9`), נבנה במלואה.** `scripts/check-text-floor.mjs` (⭆ `check-motion.mjs`) + קו בסיס קפוא `text-floor-baseline.md`. **קו הבסיס נמדד מחדש — 3 הפרות, ⛔ לא 1** (`ArenaSummary.tsx`×2·`WorldRing.tsx`×1) — הוקפאו, ⛔ לא תוקנו. ⛔ **D-110: שינוי כיוון מהתכנון —** ⛔ לא «אזהרה עם תאריך הקשחה»: `D-177` (רוי, אותו יום) אסר `SOFT_UNTIL` כליל, אז קו בסיס קפוא במקום. `verify` ⇒ **exit 0, 8 פקודות** (`test` 3207/3207 · `check:mobile` 1205/1205). `RULES.md`/`CRITIC.md`/`CONTENT.md` עודכנו (7⇢8, `rules-citations.test.ts` ירוק). `generate-map` טרי (374). | `T-236` 🟣 |
---
