<!--
NEXT_AGENT: DEV                     # ▶️ C-0847 (QA): מוזג ל-`dev`, `nav` יעד ① נמסר, המוקד `msgs` (1 ⬜ — `T-193`, CONTENT בלעדית). ⛔ 0 ⬜ כשירות ל-DEV כרגע במוקד עצמו.
...
LAST_HANDOFF_AT: "2026-09-25T17:25:42Z"   # C-0847 (QA) ⇒ DEV.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: ""                  # 🔓 C-0847 (QA) שוחררה — מוזג ל-dev, nav נחתם, מוקד→msgs, לוק שוחררה.
LOCK_AT: ""
...
#   msgs:    8 / 120           # 39 § 9 · **המוקד** (←`nav`, `C-0847` QA — היחידה עם ⬜ ברשימת «מועמדים לפי הסדר»). ⬜=1 (`T-193` CONTENT בלעדית — ⛔ אין שורה ל-PM/DEV) · 🟣=14 · ⛔=3 (`T-475`…`T-477`, `for-roy` 144). ⇒ `plan/61-deferred.md`
...
ACTIVE_WORKSTREAM: msgs        # ▶️ **הוזזה `C-0847` (QA · `STEP 5.8` · `RULES § 0.23 ז׳` ⓪).** `nav` יעד ① נמסר (חמשת המסכים, `T-499`…`T-501`·`T-503`…`T-507`, `C-0845` DEV) ⇒ ⬜=0 ⇒ שלוש החותמות נבדקו ונרשמו ב-`plan/61-deferred.md` (`C-0847`), ⛔ ולא חתימה שנייה (המקורית `C-0316` עומדת). `docs/plan-open.md` 🔴 «מועמדים לפי הסדר»: `msgs` (1⬜) הוא המועמד היחיד — `cards`/`arena`/`studies`/`amirnet` כולן מוצתות ומסומנות «ℹ️ אין פעולה». ⚠️ נספר: `msgs` ⬜=1 (`T-193`) בבעלות CONTENT בלעדית — ⛔ אין בה שורה ל-PM/DEV.
PREV_WORKSTREAM: "nav"        # `C-0847` (QA) — יעד ① נמסר, המוקד גלגל הלאה. `nav` נשארת ברוטציה (`36 § 13`).
WORKSTREAM_ENDING: ""
