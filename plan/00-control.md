<!--
NEXT_AGENT: QA                            # ▶️ C-0584 (DEV): `T-326` 🟣. `general` ⬜=0, בריכת הכשירות (+`loop`+`base`) = 10 ⬜.
STATE: READY                      # ▶️ C-0584 (DEV): טיק בנייה — `T-326`, קומיט אחד, ירוק.
...
| C-0584 | DEV | QA | 2026-09-13T19:46:52Z | 🔨 **טיק בנייה, שורה אחת.** `T-326` (‏`base`·`נוחות`) 🟣 — הפוטר המשפטי יוצא ממסכי **משימה** ונשאר בכל השאר. הרשימה נושאת את מסכי **המוצר** ⛔ ולא רק 3 הפיקסטורות; **הסתרה מוצהרת, הצגה ברירת מחדל** ⇒ הייחוס (T-011) ⛔ אינו נופל בשקט, וזמין לפני התחברות. טהור + 11 בדיקות · שער **דו-כיווני** ב-`check:mobile` · `app/layout.test.ts` (D-206) עוקב אחרי ה-`<footer>`. 🔬 הליכה 375×780, 12 מסכים: 7 מסכי משימה ⇒ **⛔ אין קישור**; `הגדרות` `712`·`/` `794`·`/sources` `2584`·`/dev/tabs/me` `744`·תיבה `730` ⇒ **יש**. ⛔ אפס גלילה · ⛔ אפס <44px. 3 שגיאות = 503 מחוסר env, מחלקה מאושרת. | `lib/core/licenceFooter.ts`(+test) · `components/SourcesFooter.tsx` · `app/layout.tsx`(+test) · `verify-mobile.mjs` · 4 רגיסטרים |
