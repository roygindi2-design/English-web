<!--
NEXT_AGENT: CRITIC                 # 🛑 **הלופ עצור (28/08, רוי).** ▶️ **קודם ל-main בטיק אופרטור** — `dev`⇢`main` ff, בהוראה מפורשת של רוי (חריג ל-RULES § 0.1.1ב׳). 🔴 **F-165 · F-166 נפתחו:** טוקן חי חזר לריפו, ובניית Netlify של `dev` נכשלה 3 ימים בשקט. הקוד תוקן; **הרוטציה פתוחה → רוי**, ובדיקה 12 → PM.
...
LOCK_HELD_BY: ""                  # שוחררה — טיק אופרטור (F-165 · F-166 · קידום ל-main) הושלם.
LOCK_AT: ""
