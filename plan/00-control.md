...
LOCK_HELD_BY: "DEV"               # 🔒 נלקחה — C-0665 (DEV, בנייה). נעילת `PM` מ-17:47:08Z נמדדה בת **195 דקות** מול תקרת 90 ⇒ נטושה (`§ 0.4`): ⛔ אפס קומיטי עבודה של PM מאז `LOCK_AT` — `C-0664` הוא קומיט הנעילה בלבד.
LOCK_AT: "2026-09-16T21:04:45Z"
...
| C-0663 | DEV | QA | 2026-09-16T17:29:32Z | 🔨 **טיק בנייה — ארבע שורות `cards`, כל התור שהיה פתוח.** `T-393` (‏סוגרת `F-271`) — פרדיקט אחד ל«לא ידעתי» במקום שניים · `T-390` — שני מספרים, שתי אוכלוסיות מוצהרות (`§ 4.2ז`) · `T-394`ⓐⓑⓓ — מסלול התקדמות ומכנה לסבב (`[role=progressbar]` 0⇢1) · … ⟨הנוסח המלא הועבר לארכיון · plan/archive/handoff-log.md · C-0663⟩ | `lib/core/deck.ts` · `lib/core/levelSummary.ts` · `lib/core/filterProgress.ts` · `app/api/study/queue/route.ts` · `components/CardDeck.tsx` · `components/DeckSelector.tsx` · `scripts/lib/walk-routes.mjs` |
