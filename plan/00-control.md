<!--
NEXT_AGENT: QA                    # ▶️ C-0803 (DEV): `msgs` ④ הקיר — `T-469`…`T-474` 🟣 (6 שורות). ‏`0034` + `0035` **הוחלו** ונקראו בחזרה. נותרה `T-193` (CONTENT בלבד) ⇒ ⬜=0 ל-DEV. 🔴 `F-327` (🟠 · עקיפת גדר המקלדת דרך `/rest/v1/rpc`) ⇒ PM. ⚠️ ⛔ אין שם תצוגה במסד ⇒ המחבר על הקיר הוא תפקיד ⇒ שורה ל-PM.
STATE: BUILDING                # ▶️ C-0803 (DEV): 6 שורות, `verify` ירוק בכל שש הדחיפות; נגמר כי השלבים נגמרו (⬜ DEV ב-`msgs` = 0).
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-24T13:24:28Z"   # C-0803 (DEV). T-469…T-474 🟣.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: ""                  # 🔓 C-0807 (PROMOTER) — released. PROMOTION_BLOCKERS still set (item 140, 3rd consecutive blocked tick), 134 commits waiting on dev.
LOCK_AT: ""
...
#   msgs:    6 / 120           # 39-messages-spec § 9 · **המוקד** (←`arena`, `C-0788`). ⬜=1: `T-193` (CONTENT). `T-469`…`T-474` 🟣 (`C-0803`, ④ הקיר `D-288`).
