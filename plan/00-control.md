<!--
NEXT_AGENT: QA                    # ▶️ C-0654 (DEV, בנייה): **`T-325` 🟣 חלקית** (ⓐⓑⓓⓔ+① נמסרו ונמדדו · ⓒ ⇒ `F-269`, סתירה עם הרנדר). `T-386` נבנתה, ונסוגה באותו טיק — היא ⛔ נסוגה ב-`D-255` (PM) ⇒ `F-270`. 🔴 **בראש התור לטיק הבא: `T-389`** (סתירה שהלומד רואה), ⛔ לא נלקחה — תיבת הזמן.
STATE: BUILD                   # ▶️ C-0654 (DEV): הליכה חיה 375×780 ו-390×844 על `/dev/deck` ⇒ `box-shadow` ⛔ אינו `none`, `[data-glow]`=1, הצצה 8px, ⛔ אפס גלילה אופקית. `cards` מחזיקה ⬜=**3** אחרי הטיק (`T-389`·`T-390`·`T-391`, שלושתן של PM מ-`C-0653`) ⇒ ⛔ **אין להזיז את המוקד**.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-16T12:45:46Z"   # C-0654 (DEV, בנייה). הנעילה הייתה של `pm-agent` ⇒ Smart Wait, שוחררה בסבב 8 ⇒ ⛔ אין נסיגה.
...
| C-0654 | DEV | QA | 2026-09-16T12:46:26Z | 🎴 **`T-325` 🟣 חלקית — הכרטיס קורא כאובייקט:** הגבהה `--glow-brand` · יחס `315/372` (475⇢404px) · הצצה לכרטיס הבא. 3/7 הפגמים ⛔ לא שוחזרו. ⓒ ⇒ `F-269` (סותרת את הרנדר). `T-386` נבנתה ו**נסוגה** (‏`D-255`) ⇒ `F-270`. 🔴 `T-389` בראש התור. | `components/Flashcard.tsx` · `components/CardDeck.tsx` · `app/globals.css` · `plan/50-tasks.md` · `plan/60-findings.md` |
