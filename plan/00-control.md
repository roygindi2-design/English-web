<!--
NEXT_AGENT: QA                        # ▶️ C-0547 (DEV) — `T-297` 🟣 (מאגר פריטי אמירנט + הנתיב שמגיש אותו; מיגרציה `0024` הורצה חי ואומתה). ⬜ פנויות ל-DEV: `T-270` (amirnet) · `T-302`·`T-303` (base).
STATE: READY                      # ▶️ C-0547 (DEV, טיק בנייה) — `T-297` 🟣: `public.amirnet_items` חיה (0 שורות, RLS), `GET /api/amirnet/practice` רך, ו-`toServedItems()` שממפה ו⛔ אינו מתקן.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-12T16:42:24Z"   # C-0547 (DEV, קריאה קדימה ל-`amirnet`) — `T-297` 🟣, מיגרציה 0024 הורצה חי, verify ירוק.
HUMAN_DECISION_REQUIRED: false    # ▶️ ⛔ אינו ממתין. **44 · 45 · 46 נסגרו כולם ב-23/08.** נותר **47** — שורה ב-`RULES § 0.1 ב׳` שהיא הקובץ של רוי, ו⛔ **אינה חוסמת את הקידום של היום**.
# --- נעילה: מונעת שני סוכנים שכותבים לקובץ בו-זמנית ---
LOCK_HELD_BY: "DEV"              # 🔒 נלקחה 2026-09-12T16:32:45Z — C-0547 (DEV, טיק בנייה): `T-297` — מאגר פריטי אמירנט + הנתיב שמגיש אותו (`41 § 6.5` · `D-212`).
LOCK_AT: "2026-09-12T16:32:45Z"
...
#   amirnet: 10 / 120          # 41 § 8-1..3 · ▶️ C-0547 (DEV, **קריאה קדימה** — `msgs` נשאר ACTIVE_WORKSTREAM ועמד על ⬜=1 של CONTENT בלבד): `T-297` 🟣 — `public.amirnet_items` (‏`0024`, הורצה חי דרך מחבר ה-MCP ואומתה ב-`list_tables`: קיימת · RLS · 0 שורות) + `GET /api/amirnet/practice`. ⬜ פנויות ל-DEV: `T-270` בלבד. ⚠️ המאגר **ריק** — הכתיבה אליו היא `K-006` (CONTENT). ⟨C-0545 (CONTENT): K-005/K-007, 30 שורות Tier 1, 684→714/3,382.⟩ ⟨C-0544 (DEV): `T-298` 🟣 — מסך תוצאת הסימולציה.⟩ ⟨C-0538 (DEV): `T-296` 🟣 — מנוע הסימולציה.⟩
