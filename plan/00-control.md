...
```yaml
CYCLE_ID: C-0082                  # מזהה רץ. עולה ב-1 בכל מעבר PM→Dev→Critic→PM
NEXT_AGENT: PM                    # **C-0082 (CRITIC): סקירה מהירה נקייה.** ארבע פקודות ירוקות בהרצה שלי: typecheck ✅ · check:core `/lib/core purity: OK` ✅ · **796 בדיקות ✅ (53 קבצים)** · build ✅. סומנו ✅: **T-051** (מעטפת ניווט) · **T-058** (עיגון עליון). T-003 נשאר 🔵 (משימה 4/4 פתוחה, בלוק "המטרה שלך" ב-`/me`). ⛔ **לא קידמתי — וזו אינה בעיית ממצא.** `merge --ff-only dev` נכשל: `main` מכיל את `4ddf6c4` (עריכה ישירה של רוי ל-`02-inbox.md`, 12/08) שאינו ב-`dev`. איחוי `main`→`dev` עדיין מתנגש ב-`plan/02-inbox.md` — **קובץ PM, אסור ל-Critic לגעת.** 🔴 **פעולת PM נדרשת (התור שלך ב-08:00, הראשון מאז שהוסף הדגל ב-C-0074):** `git merge origin/main` ל-`dev`, פתור התנגשות `02-inbox.md` (שמור 16 שורות רוי + מצב dev), דחוף `dev`. אז Critic יקדם ב-ff-only. ⛔ לא נגעתי ב-`main`.
STATE: BUILDING                   # PLANNING | BUILDING | REVIEWING | BLOCKED | MILESTONE_DONE
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: T-003             # 🔵 **משימות 1–3/4 הושלמו (C-0079 · C-0080 · C-0081).** הבאה: **משימה 4** — בלוק "המטרה שלך" ב-`components/MeScreen.tsx` + קריאת `profiles` ב-`app/(tabs)/me/page.tsx`. אחריה **T-056** (5) · **T-057** (6). ⛔ לעולם לא לכתוב ל-`senses.cefr_level`. ⚠️ T-007 (NGSL) עדיין ⛔ חסום ב-T-043. T-059 ⬜ תיעוד בלבד.
CRITIC_ROUNDS_ON_TASK: 0          # תור הסקירה: T-005 · T-006 · T-018 · **T-052 + F-027 (C-0071)** · **T-003 משימות 1–3 (נסקרו C-0082 — תקינות ברכיב; המשימה 🔵 עד משימה 4)** · ✅נסגרו C-0082: T-051 · T-058 (F-030 כבר סגור)
CONSECUTIVE_NO_PROGRESS: 0        # תקרה: 2 → מעבר אוטומטי ל-HUMAN. C-0081 התקדם: 789 → 796 בדיקות, משימה 3/4 נסגרה
LAST_HANDOFF_AT: 2026-08-13T07:11:30Z
...
LOCK_HELD_BY: DEV                 # "" | PM | DEV | CRITIC | CONTENT — נתפסה C-0083
LOCK_AT: "2026-08-13T07:37:55Z"
MILESTONE_TICKS: 36
