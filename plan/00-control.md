<!--
NEXT_AGENT: QA                     # ▶️ C-0644 (DEV): **בנייה — 3 שורות `story` נמסרו (`T-383`·`T-381`·`T-382`), קומיט לכל אחת.** `verify` מלא ירוק (4,720 בדיקות · `check:mobile` 1,985). ⇒ `story` ⬜=**4** · 🟣=**7**.
STATE: BUILDING                   # ▶️ C-0644 (DEV): הליכה חיה ב-320/375/414 (`next start`) על `/dev/story` · `/dev/story/done` ועל מצב הטעינה עם API מושהה — ⛔ 0 גלילה אופקית · ⛔ 0 יעד מתחת ל-44px · ⛔ 0 שגיאות קונסול.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: []   # ▶️ C-0529 — ריק.
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
LAST_HANDOFF_AT: "2026-09-16T06:49:00Z"   # C-0644 (DEV, בנייה). הנעילה הייתה פנויה בפתיחה ⇒ ⛔ אין Smart Wait ו⛔ אין נסיגה.
...
#   story:  19 / 120           # § 13-1 · **המוקד**. ⬜=4 · 🟣=7 · ⛔=0 (נמדד ב-`docs/plan-open.md` אחרי C-0644). הנרטיב ⇒ `plan/archive/control-log.md`
...
WORKSTREAM_ENDING: ""             # 🟢 **נשאר ריק — מדידה `C-0644`, ⛔ ולא הערכה.** `story` ⬜=**4** (`T-208`·`T-266`·`T-269`·`T-380`) ⇒ **>0** ⇒ המוקד ⛔ אינו זז, ו-`§ 0.23 ז׳` ③ חל (‏`story` היא מחלקה ברצף, ⛔ ולא חדר המתנה). ⛔ אינו חותם ו⛔ אינו מבטל חותמת.
