<!--
NEXT_AGENT: CRITIC                 # ▶️ C-0435 (DEV) — **טיק תכנון בלבד, אפס קוד.** `T-223` (`base`·תשתית) שוחררה ב-C-0433 (`T-222` נחתה) אבל השורה עצמה מציינת **היקף חדש (מודול טהור + שער פרק + סקריפט מדידה), STEP 3 מחייב טיק תכנון נפרד** — ⛔ לא נבנתה כאן. נכתבה `docs/superpowers/plans/2026-09-05-amirnet-item-gate.md`: שלוש משימות — ⓐ `lib/core/amirnetItemGate.ts` (שער פר-פריט, 11 סוגי כשל, כולל תקרת Tier-אוצר-מילים ואורך קטע `rc`) ⓑ `lib/core/amirnetChapterGate.ts` (5 שאלות לפרק, פיזור `correct_index`) ⓒ `scripts/measure-amirnet-gate.mjs` + `npm run measure:amirnet-gate` (F-163). כל בלוק קוד בתוכנית הורץ ידנית מחוץ לריפו (node, לא vitest) לפני הכתיבה הסופית — כולל תיקון אורך קטע (131→188 מילים, בטווח רמה 3 [180,230]) ותיקון תלות-סביבה (`AMIRNET_VOCAB_CSV` env override, כדי שבדיקות ה-CLI לא ייפלו על אוצר מילים אמיתי שמשתנה). **קריאה הפיכה שלי (RULES § 0.22):** סף `1.4` ליחס אורך-תשובה-נכונה/חציון-מסיחים הוא ברירת מחדל סבירה של DEV — המפרט דורש "מתחת לסף" בלי מספר. `ACTIVE_TASK_ID` (`T-253`) נבדק כנדרש ב-STEP 2 — `grep -n '^\| T-253 \|' plan/50-tasks.md` עדיין מחזיר `✅ C-0428 (QA): על dev`, אותו מצב תקוע בדיוק. ⛔ לא נוספה שורה חמישית זהה לשדה `ACTIVE_TASK_ID` למטה — כבר תועד פעמיים ב-03-for-roy #96, ושורה נוספת הייתה רק מנפחת קובץ שכבר מעל התקרה בלי מידע חדש.
STATE: BUILD                      # ▶️ C-0422 (PM) — נשאר BUILD. **17 ⬜ כשירות ל-`general`** (בדיקה 11), ובהן `T-253` שנפתחה בטיק.
ACTIVE_MILESTONE: M0              # M0..M6
ACTIVE_TASK_ID: T-253             # ⚠️ ▶️ C-0429 (DEV) — **נמדד: מצביע תקוע.** `grep -n '^\| T-253 \|' plan/50-tasks.md` ⇒ תא סטטוס `✅ C-0428 (QA): על dev` — השורה כבר נמסרה ונמזגה **לפני** תחילת הטיק הזה. ⛔ DEV לא כותב לשדה הזה בעצמו (RULES); דווח ב-`03-for-roy` שהמצביע דורש שורה כשירה חדשה מ-PM/QA. ⚠️ ▶️ C-0430 (DEV) — נמדד שוב, אותו מצב בדיוק (`✅ C-0428 (QA): על dev`); ⛔ לא נגעתי בשדה. ⚠️ ▶️ C-0433 (DEV) — נמדד פעם שלישית, אותו מצב בדיוק; ⛔ לא נגעתי בשדה. ⚠️ ▶️ C-0434 (DEV) — נמדד פעם רביעית, אותו מצב בדיוק (`✅ C-0428 (QA): על dev`); ⛔ לא נגעתי בשדה. כבר מתועד ב-`03-for-roy` פריט 96 (נוסף שם, לא כאן, כדי לא לנפח קובץ שכבר מעל התקרה).
CRITIC_ROUNDS_ON_TASK: 0          # ⛔ התור הישן הועבר ל-`plan/archive/control-log.md` (26/08) — הוא היה מת: QA הופך 🟣⇢✅ בכמות מ-`git log`.
CONSECUTIVE_NO_PROGRESS: 0        # תקרה: 2 → מעבר אוטומטי ל-HUMAN
LAST_HANDOFF_AT: 2026-09-05T06:50:42Z
...
LOCK_HELD_BY: ""                    # שוחררה C-0435 (DEV) 06:50:42Z — טיק תכנון בלבד, `docs/superpowers/plans/2026-09-05-amirnet-item-gate.md` נכתבה ונדחפה, אפס קוד.
LOCK_AT: "2026-09-05T06:47:00Z"
...
> 🧹 `C-0430` הועבר 05/09/2026 (C-0433) · `C-0432` הועבר 05/09/2026 (C-0434) · `C-0433` הועבר 05/09/2026 (C-0435) — כולן שתי שורות בלבד, פירוט המדידה בארכיון (`plan/archive/control-log.md`).
