...
describe('<StudiesScreen> — הבורר הגולש אומר שהוא גולש (T-330)', () => {
  ...
  /**
   * T-414 — הבדיקה הזאת קיבעה `px-3`, ו**`px-3` הוא מה שדחף את השבב הרביעי
   * לשורה שנייה ב-375px**: 319.9px שבבים + 24px רווחים = 343.9 מול רצועה של
   * 327 (נמדד חי, `C-0691`/`C-0695`). ⇒ היא ⛔ לא נשברה בשקט — היא נכתבה מחדש
   * סביב מה שבאמת אסור לזוז: **הרצפה**, ⛔ ולא הריפוד.
   */
  it('ⓒ השבבים על רצפת ה-12px ו⛔ לא מתחתיה, והריפוד הוא מה שנותן (§ א9 · T-414)', () => {
    // `render_video_A.py:1277-1283` — 12.5px, ריפוד 13px לצד. `text-xs` = 12px,
    // כלומר **על** הרצפה ⛔ ולא מתחתיה. הריפוד ירד ל-`px-2` כי הוא המשתנה
    // היחיד שנשאר: ⛔ אין קיצור שם (`36 § 9`), ⛔ אין טקסט קטן יותר (הרצפה),
    // ו⛔ אין `overflow-x-auto` (`T-410`).
    const chips = CODE.match(/'flex min-h-touch shrink-0[^']*'/g) ?? [];
    expect(chips).toHaveLength(2);
    for (const chip of chips) {
      expect(chip).toContain('text-xs');
      // ⛔ הרצפה ⛔ אינה נפתחת כדי לקנות רוחב — `text-[11px]` ⛔ ולא כל וריאנט קטן.
      expect(chip).not.toMatch(/text-\[(?:\d|10|11)px\]/);
      expect(chip).toContain('px-2');
      expect(chip).not.toContain('px-3');
      // ⛔ יעד ההקשה ⛔ אינו יורד עם הטיפוגרפיה — 44px הוא שער קפוא.
      expect(chip).toContain('min-h-touch');
    }
  });

  it('ⓒ רווח הרצועה נשאר 8px — בדיוק כפי שהרנדר מצייר אותו (T-414)', () => {
    // `render_video_A.py:1285` — `x -= w + 8`. ⇒ `gap-2` ⛔ אינו בחירה, הוא
    // המספר של הרנדר, והקיצוץ נלקח כולו מהריפוד ⛔ ולא ממנו.
    expect(CODE).toMatch(/role="tablist"[\s\S]{0,200}?className="flex flex-wrap gap-2"/);
  });
});
