...
/**
 * 🔢 **⟦`T-390` · `§ 4.2ז`⟧ «בכל הרמות» ⛔ אינו קישוט — הוא ה**אוכלוסייה** של המספר.**
 *
 * המספר הזה מגיע מ-`deck=unknown`, שמסונן ב-`user_id` **בלבד** ⇒ הוא סופר על פני כל
 * שש הרמות; המונה «לא ידעתי ברמה» שני אריחים מעליו מגיע מ-`LevelSummary` ⇒ **רמה אחת**.
 * ⇒ שני המספרים רשאים להיות שונים **גם כששתי הקריאות מצליחות**, ועד `T-390` ⛔ שום דבר
 * במסך ⛔ לא אמר ללומד למה. ⛔ זו ⛔ אינה `T-389` (שם הקריאה **נכשלה**).
 * ⚠️ **ותקרת `MAX_QUEUE_ROWS` (200) ⛔ אינה נאמרת ללומד:** היא תקרה על **שורות שנקראו**,
 * ⛔ ולא על האוכלוסייה — ומספר תקרה בקופי הוא רעש, ⛔ לא דיוק.
 */
const PRACTICE_NOTE_HE = (n: string) => `${n} מילים שסימנת לא ידעתי, בכל הרמות`;
