...
export default function CardDeck({
  ...
  /**
   * 📍 **⟦`T-400` · `F-272` · `D-260`⟧ «נשארו N מילים ברמה» — המספר השני שהרנדר מצייר.**
   *
   * 🔬 **נמדד ב-`render_video_A.py:391`, ⛔ ולא נבחר:** מתחת לשני כפתורי הדירוג יושבת
   * שורה אחת, ממורכזת, ב-12.5px ובגוון `INK_MUTED` מעומעם — «5 מתוך 20 · נשארו 314 מילים
   * ברמה». חציה הראשון כבר על המסך מאז `T-394` (בכותרת, הכרעה מוצהרת); **החצי השני
   * ⛔ לא היה בשום מקום**, וזה מה שהשורה הזאת מביאה.
   *
   * ⛔ **אופציונלי, ⛔ ומסיבה נמדדת:** רק חפיסת `סינון מילים` מוגדרת על ידי רמה
   * (`T-155` · `D-089`). ל-`מנת היום` ול-`לא ידעתי` ⛔ אין «רמה» שמשהו נשאר בה, ומספר
   * כזה עליהן היה טענה שגויה. ⇒ הצרכן היחיד שמעביר אותו הוא `<StudyDeckScreen>`, ורק
   * כשהשרת החזיר אותו — ⇒ הפיקסצ׳רים והנתיבי-`/dev` הקיימים ⛔ אינם נשברים כשהוא חסר.
   *
  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden" data-deck-viewport>
        {/* 🎴 **`T-325`ⓓ — הכרטיס הבא מציץ מאחור, ⛔ כי «נותרו 5» היה הערוץ היחיד.**

            🔬 **נמדד בטיק הזה ב-390×844 על `/dev/deck`, ⛔ ולא בקריאת קוד:** הדק מרנדר
            `remaining.slice(0, 1)` — כרטיס **אחד** — ⇒ ⛔ אין ולו רמז חזותי אחד לכך
            שמאחוריו יש עוד. מה שכן היה: המחרוזת «נותרו 5» בכותרת. ⇒ **הערימה הייתה
            מספר, ⛔ ולא צורה** (פגם ⑤ בשורה).

            ⛔ **⛔ ואינו כרטיס שני על הבמה:** `aria-hidden` ו-`pointer-events-none` ⇒
            קורא-מסך ⛔ אינו שומע אותו והאצבע ⛔ אינה פוגשת אותו — «נותרו N» נשאר
            הערוץ הנגיש, בדיוק כפי שהיה. ⛔ ואינו נושא `data-glow`: ההגבהה (ⓐ) שייכת
            לאובייקט ש**על** הבמה, ⛔ ולא לרמז מאחוריו — ⛔ ושני זוהרים היו אוכלים את
            כל התקציב של החוקה (ב3) על מסך אחד.
      ...
      {/* 📍 **⟦`T-400`⟧ שורת הכף־רגל — המספר השני של `kol-A-03-card.png`.**

          🔬 **נמדד ב-`docs/design/render_video_A.py:391`, ⛔ ולא שוער:**
          `c.txt(LW/2, y + h + 34, "5 מתוך 20 · נשארו 314 מילים ברמה", 12.5, "Regular",
          INK_MUTED)` — שורה אחת, **ממורכזת**, **מתחת** לשני כפתורי הדירוג, בגודל הקטן
          ביותר שבמסך. ⇒ המיקום, היישור והגוון נלקחו מהרנדר; רק **הגודל** עוגל
          מ-12.5px ל-`text-xs` (12px), שהוא הרצפה הקשיחה של החוקה `§ א9` והשלב
          הקרוב ביותר בסולם. ⛔ ו⛔ בלי `opacity` על הטקסט: הרנדר מצייר אותו ב-65%
          שקיפות, ורצפת הניגודיות היא **שער קפוא** ⇒ `--ink-muted` מלא, ⛔ ולא מעומעם.

          ⛔ **ו⛔ אינו «נותרו N» שני.** «נותרו» בכותרת סופר את **הסבב** — כמה כרטיסים
          נשארו בערימה שעל השולחן; זה סופר את **הרמה** — כמה מילים הלומד עוד לא פגש
          בכלל. שני מספרים, שני מכנים, ⇒ המילה «ברמה» היא מה שמפריד ביניהם.
          ⛔ **ואינו נקודה שלישית:** ⛔ אין כאן `·` — טעם-הטיפוגרפיה מקצה נקודה אמצעית
          אחת לשורה, והשורה שבכותרת כבר משתמשת בשלה.

          ♿ **ו-`role="status"` ⛔ אינו קישוט:** המספר משתנה בין טעינות ⛔ ולא בתוך הסבב
          (הוא ⛔ אינו יורד עם כל דירוג — «טרם נראה» היא עובדת מאגר, ⛔ ולא מונה סבב),
          ⇒ הוא נקרא פעם אחת כשהמסך נוחת ו⛔ אינו מקשקש. ⛔ **ואפס תנועה** —
          טקסט סטטי, ⛔ ללא `transition` ⇒ `prefers-reduced-motion` מכובד **בבנייה**.

          ⚠️ **ו-`flex-none`, ⛔ ולא ילד של `[data-deck-viewport]`:**
          `scripts/verify-mobile.mjs` מודד שכל **ילדי** הבמה ממלאים אותה, וילד נמוך
          מאדים את השער בשלוש המידות (`T-086` · `C-0104`). ⇒ השורה היא **אחות** של
          הבמה, והבמה (`min-h-0 flex-1`) מתכווצת סביבה מעצמה. */}
      {typeof unseenInLevel === 'number' && (
        <p
          role="status"
          data-level-unseen={unseenInLevel}
          className="flex-none pt-2 text-center text-xs text-ink-muted"
        >
          נשארו {unseenInLevel} מילים ברמה
        </p>
      )}
    </section>
  );
