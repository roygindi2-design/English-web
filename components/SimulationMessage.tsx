...
export const SENT_HE = 'נשלח';
export const ALREADY_ANSWERED_HE = 'כבר ענית על ההודעה הזאת';
export const SEND_FAILED_HE = 'השליחה לא הצליחה. הבלוקים נשארו כאן, אפשר לנסות שוב.';
...
/**
 * T-462ⓒ — after a send, the keyboard gives way to what was sent. The chips above keep
 * the state they had at send time. ⛔ The reply text is ⛔ not stored (no column), so a
 * reload shows `ALREADY_ANSWERED_HE` and ⛔ not the sentence.
 */
export function SentPanel({ words }: { readonly words: readonly string[] | null }) {
  return (
    <div
      data-sent-panel
      className="flex min-h-touch items-center gap-2 rounded-2xl border border-success bg-surface-raised px-3 py-2 text-sm text-ink"
    >
      <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 shrink-0 text-success" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M5 12.5l4.5 4.5L19 7.5" />
      </svg>
      <span className="font-bold text-success">{words === null ? ALREADY_ANSWERED_HE : SENT_HE}</span>
      {words !== null ? (
        <span className="min-w-0 flex-1 truncate font-semibold">
          <EnWord>{words.join(' ')}</EnWord>
        </span>
      ) : null}
    </div>
  );
...
export default function SimulationMessage({ id }: { readonly id: string }): React.JSX.Element {
  ...
  const [send, setSend] = useState<
    { kind: 'idle' } | { kind: 'sending' } | { kind: 'sent'; words: readonly string[] } | { kind: 'failed' }
  >({ kind: 'idle' });
  const onSend = useCallback(
    (words: readonly string[]) => {
      setSend({ kind: 'sending' });
      void apiPost<{ ok: boolean }>(`/api/world/messages/${id}/answer`, { words })
        .then((body) => setSend(body.ok ? { kind: 'sent', words } : { kind: 'failed' }))
        .catch(() => setSend({ kind: 'failed' }));
    },
    [id],
  );
  ...
  return (
    <SimulationMessageView
      ...
      keyboard={
        state.kind !== 'ready' ? null : send.kind === 'sent' ? (
          <SentPanel words={send.words} />
        ) : state.item.answeredAt !== null ? (
          <SentPanel words={null} />
        ) : (
          <>
            {send.kind === 'failed' ? (
              <p role="alert" className="mb-2 text-xs text-ink">
                {SEND_FAILED_HE}
              </p>
            ) : null}
            <BlockKeyboard
              ...
              onChosen={onChosen}
              onSend={send.kind === 'sending' ? undefined : onSend}
            />
          </>
        )
      }
