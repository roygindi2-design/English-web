...
describe('T-402 — הסיבה למצב המושבת נקראת בלי גלילה', () => {
  it('עמודה **מדויקת** ⛔ ולא מינימום — התקדים של `ArenaBattle`/`CardDeck`', () => {
    /**
     * 🎬 **⟦17/09 · `C-0709` · `T-423`ⓑ⟧ העמודה מדויקת כשהייתה, ⛔ והמספר הוא עכשיו
     * `100dvh` **מלא** — ⛔ כי ה-84 שניכתה כאן ⛔ אינם על המסך יותר.**
     *
     * 🔬 **וזו מדידה, ⛔ ולא העדפה:** `app/arcade/arcade-tokens.css` מוריד את
     * ה-`<header>` (52) ואת `pb-8` של ה-`<main>` (32) בכל מסמך שנושא
     * `data-arena-scope`, והמקטע הזה נושא אותו. ⇒ ניכוי של 84px שכבר ⛔ אינם קיימים
     * היה משאיר **84px של כלום** בתחתית — כלומר בדיוק הפגם ההפוך מזה ש-`T-402`
     * נכתב נגדו, ⛔ ולא תיקון שלו.
     * ⛔ **ומה ש-`T-402` מדד ⛔ לא הוחלש:** גובה **מדויק** ⛔ ולא מינימום, ⛔ אפס
     * `pb-16`, והכרטיסים הם מה שגולל. שלוש הטענות למטה ⛔ לא נגעו.
     */
    expect(CODE).toMatch(/h-\[100dvh\]/);
    // ⛔ והניכוי ⛔ אינו חוזר — הכרום ⛔ אינו קיים במסלול הזה.
