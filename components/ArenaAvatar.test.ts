...
describe('<ArenaAvatar>', () => {
  /**
   * 🎨 **⟦23/09 · `C-0763` · `D-272` · הוראת רוי⟧ נכס גרפי מבחוץ — **מותר**.**
   *
   * הטענה הקודמת אסרה כל תמונה (`<img>` · `.png` · `url(…)`) בשם «תקציב אפס». רוי הסיר
   * את הבלם: «אפשר להסיר אותו, שלא יהיה סתם — זה פוגע». ⇒ ⛔ אין עוד איסור על נכס.
   * ⚠️ **ושני דברים נשארים, כי הם ⛔ אינם הבלם:**
   * ⓐ ⛔ **קישור חיצוני** — נכס נכנס לריפו ⟨`public/`⟩. `check:mobile` מכשיל כל בקשה
   *    שנכשלת, ונכסים מקושרים מבחוץ ⟨למשל Figma⟩ פגים. ⇒ ⛔ `https://` ברכיב.
   * ⓑ ⛔ **אמוג'י** — הגליף מגיע מהגופן ⛔ ולא מהקוד, ונראה אחרת בכל גופן.
   */
  it("⛔ אפס אמוג'י · ⛔ אפס קישור חיצוני — ונכס גרפי מבחוץ **מותר** כשהוא בריפו", () => {
    expect(CODE).toMatch(/<svg/);
    expect(CODE).not.toMatch(/https?:\/\//);
    expect('src="https://cdn.example/x.png"').toMatch(/https?:\/\//);
