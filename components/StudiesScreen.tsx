...
/**
 * `T-513` · `F-337` — הפעימה **חוזרת כל עוד השלד על המסך.** ⟦תוקן: הייתה `_1`, פעימה
 * אחת⟧ 🔬 טעינה קרה של המסך הזה נמדדה ~3.7 שניות (`F-255`) מול פעימה של 2 ⇒ ~1.7 שניות
 * של בלוק אפור קפוא, שנקרא «נתקע» ⛔ ולא «טוען». ‏`infinite` ⛔ אינו שורד את הטעינה: השלד
 * יורד מה-DOM ברגע ש-`loading` נגמר. ‏`motion-safe` ⇒ תחת `prefers-reduced-motion` השלד סטטי.
 */
const SKELETON_PULSE = 'motion-safe:animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite]';
/**
