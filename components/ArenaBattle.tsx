  ...
  return (
    ...
    >
      ...
      {/* ⟦17/09 · `C-0709` · `T-423`ⓑ⟧ `deck` — **196px מוצהרים** (`3319:16`).
          ...
          מה שנגרע ⛔ ולא התוכן. מעל 700px גובה ⛔ שום דבר ⛔ אינו משתנה, ⇒ שתי המידות
          ש-ⓒ טוען עליהן (852 · 932) ⛔ לא נגעו.
          📏 **⟦`T-459` · `C-0786`⟧ ⛔ 196 ⛔ ו-140 שניהם יצאו ⇒ `120px`, בכל גובה.** 🔬 **נמדד
          ב-Chromium, ⛔ ולא שוער:** הקלפים שלנו 100px (‏⛔ לא 132 של Figma) ⇒ תחתית הקלף
          יושבת **111px** מתחת לראש הרצועה (‏`pt-2` + 100 + מרווח המניפה של `T-458`), ו-196
          השאירו **~97px ריקים** מעל «יכולות» — הפוך מ-`kol-B-03-battle.png`, שבו היכולות
          צמודות לקלפים. ⇒ 120 = התוכן + ~9px. ⛔ **ה-76 ⛔ אינם נעלמים:** הבמה (‏`flex-1`)
          בולעת אותם, והסכום נשאר `100dvh` (‏`D-268`) ⇒ 406 לבמה ב-393×852.
          ⚠️ **מתחת ל-375 הקלף 76px** (‏`SpellCard` · `min-[375px]:h-[100px]`) ⇒ 120 השאירו שם
          31px; ‏`96` = אותו חשבון על קלף של 76. */}
      <ul
        ref={handRef}
        data-arena-hand
        className="grid h-[96px] shrink-0 grid-cols-4 content-start gap-2 pt-2 min-[375px]:h-[120px]"
      >
        {hand.map((option, i) => (
          <li key={option.he}>
