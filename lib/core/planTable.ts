...
const STATE_GLYPHS: ReadonlyArray<readonly [string, TaskState]> = [
  ...
  ['🔓', 'open'],
  /**
   * 🔵 **⟦20/09 · `C-0742` · `F-308`⟧ «בעבודה» — הגליף שה**מקרא מתעד** ו⛔ שלא
   * היה כאן.**
   *
   * 🔬 **נמדד, ⛔ ולא שוער:** `plan/50-tasks.md:5` מתעד «⬜ ממתין · **🔵 בעבודה** ·
   * 🟣 בביקורת · ✅ הושלם · ⛔ חסום», ו-`scripts/loop-health.mjs:279` מונה אותו —
   * ⛔ אבל הטבלה הזאת, שהיא **המסווג הקנוני**, ⛔ לא. ⇒ שורה שסומנה כדין נפלה
   * ל-`❔ סטטוס שאינו מזוהה` ו**חסמה דחיפה**.
   *
   * 🔴 **והנזק היה גרוע מ«לא מזוהה», וזה מה שהכריע:** `T-184` נושאת `🔵` ראשון,
   * ⇒ הגליף הראשון ה**מוכר** בתא שלה הוא `⛔` שיושב ב**פרוזה** («⛔ החצי השני
   * עדיין פתוח»), ⇒ היא דווחה **`blocked`** — שורה שהיא **בעבודה** נספרה כחסומה
   * במשך שבועות. ⛔ `⛔` הוא גם גליף סטטוס **וגם** סימן השלילה של הפרוזה כאן.
   *
   * ⇒ `open`, ⛔ ולא מצב שישי: «בעבודה» היא שורה ש**נבחרה ורצה**, וזה בדיוק מה
   * שהתור `open` אומר. ⛔ מצב שישי היה דורש תור שישי שאיש ⛔ אינו קורא.
   */
  ['🔵', 'open'],
  ['⛔', 'blocked'],
  ['🚫', 'cancelled'],
