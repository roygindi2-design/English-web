...
describe('D-059 · D-067 — שלושת קבועי הקרב, והסף שנגזר מהם', () => {
  ...
  /**
   * ⚠️ **התהפך ב-T-158 (D-090ⓒ · `03-for-roy` פריט 39).** רוי אישר «חריג צר» עם
   * ברירת מחדל כבויה, וההיפוך הוא שורה אחת — ⛔ אבל אך ורק אחרי ש-**T-119** הנחיתה
   * את הצרכן: דגל בלי ולו קורא אחד הוא **שינוי אפס** ללומד, וזו בדיוק הסיבה שהשורה
   * הזאת ישבה חסומה. הגדר עצמה נמדדת ב-`components/ArenaStage.test.ts`, ⛔ לא כאן.
   */
  it('D-060 · פריט 39 · D-090ⓒ — לולאת ההמתנה דלוקה, אחרי שנחת לה צרכן (T-119)', () => {
    expect(ARENA_IDLE_LOOP).toBe(true);
  });
});
