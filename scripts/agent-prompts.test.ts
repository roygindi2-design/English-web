...
describe('docs/agents/*.md — הפרומפטים הם קובץ בריפו, ⛔ לא סוד במשימה מתוזמנת', () => {
  it('חמשת הפרומפטים קיימים, ו⛔ אין קובץ שישי שאיש לא מכיר', () => {
     ...
     */
    /**
     * 🧰 ⟦17/09 · `C-0692`⟧ **`TOOLS.md` הוא חוזה כלים, ⛔ ולא פרומפט שביעי.** הוא ⛔ אינו
     * נקרא בכל טיק ו⛔ אינו נושא STEP — הפרומפטים מפנים אליו לפי כלי. הטענה למטה שומרת
     * שהוא יישאר כזה: ⛔ בלי «STEP» ו⛔ בלי לוח זמנים, שזה מה שמפריד «חוזה» מ«פרומפט שביעי
     * שסוטה» — בדיוק ההפרדה ש-`CRITIC.md` מקבלת שורה מתחת.
     */
    expect(found).toEqual([...ALL_PROMPTS, 'CRITIC', 'TOOLS'].sort());
    const tools = readFileSync(join(DIR, 'TOOLS.md'), 'utf8');
    expect(tools, '⛔ החוזה ⛔ אינו הופך לפרומפט').not.toMatch(/^##\s*STEP /m);
  ...
  describe('📇 אינדקס הסקילים — `docs/skills-registry.md`  ⟦C-0376⟧', () => {
    ...
    /**
     * 🧰 ⟦17/09 · `C-0692`⟧ **הערוץ שבו PM מכוון את DEV לכלי — ⛔ ולא רק לסקיל.**
     * 🔬 **נמדד:** התא `סקיל` ב-`plan/50-tasks.md` הוא `—` ב-**395 מתוך 409 שורות** ⇒ הערוץ
     * בנוי מאז 31/08 ו**ריק ב-98%**. ⇒ ההוראה החדשה ⛔ אינה «עוד שדה», היא **שתי חובות**:
     * ⓐ בשורת מסך `—` ⛔ אינה תשובה זמינה, ⓑ ומה שנכתב הוא **ראיה**, ⛔ ולא שם כלי —
     * «`Kernel`» ⛔ אינה שורה, «`Kernel`: 393×852 ⇒ אפס גלילה» היא.
     *
     * ⛔ **ומה שהטענה הזאת ⛔ אינה עושה, בכוונה:** היא ⛔ אינה סורקת את `50-tasks.md`.
     * 395 השורות הישנות נושאות `—` כדין לפי הכלל שהיה תקף כשנפתחו, וגדר שמאדימה על
     * היסטוריה היא גדר שמכבים. ⇒ האכיפה על שורות **חדשות** היא של PM, והחוב רשום.
     */
    /**
     * 🚚 ⟦18/09 · `C-0715` · `F-289`⟧ **CONTENT חייב לסגור את המסלול עד המסד.**
     * 🔬 **נמדד `C-0712`:** המסד החזיק 476 מילים והאצוות 1,483 ⇒ **37 אצוות, חודש
     * שלם, ⛔ מעולם ⛔ לא הוחלו**. הצינור ⛔ לא היה שבור — הוא פשוט ⛔ לא הורץ, ו⛔ אף
     * שער ⛔ לא ידע, כי כולם מודדים את הריפו ו⛔ אף אחד ⛔ לא את הפער מול המסד.
     * ⇒ הטענה הזאת נועלת את שלושת החלקים שבלעדיהם הפער חוזר: הפקודה שמראה אותו,
     * הכלי שמחיל (⛔ ולא `apply_migration`, שנדחה במסווג), ומלכודת ה-`null` בפס
     * ה-CEFR — שבלעדיה מילה **מוחלת** עדיין ⛔ אינה מגיעה ללומד.
     */
    it('ⓓ₃ 🚚 CONTENT סוגר את המסלול עד המסד, ⛔ ולא עד הקובץ', () => {
      const c = text('CONTENT');
      expect(c, 'הפקודה שמראה את הפער').toContain('ingest:status');
      expect(c, 'הכלי שמחיל').toContain('mcp__Supabase__execute_sql');
      expect(c, 'ו⛔ לא זה שנדחה').toMatch(/`apply_migration` ⛔ אינו הכלי כאן/);
      expect(c, 'מלכודת הפס — בלעדיה המילה ⛔ אינה מגיעה').toContain('cefr_profile_band is null');
      expect(c, 'הדוח נושא מספר').toMatch(/«כתבתי אצווה» ⛔ אינה תפוקה/);
    });

    it('ⓓ₂ 🧰 PM נדרש לכתוב ראיה בתא, ו⛔ `—` ⛔ אינה זמינה בשורת מסך', () => {
      const pm = text('PM');
      expect(pm, 'PM: התא נושא גם כלי').toContain('סקיל · כלי');
      expect(pm, 'PM: ראיה ⛔ ולא שם כלי').toMatch(/ואתה כותב ראיה, ⛔ ולא שם כלי/);
      expect(pm, 'PM: ⛔ אין `—` בשורת מסך').toMatch(/⛔ אינה תשובה זמינה/);
      // ⛔ ושהרישום ⛔ אינו נדרש עוד במלואו — 18,500 ב׳ בכל טיק תכנון.
      expect(pm, 'PM: ⛔ לא עוד «whole»').not.toMatch(/MUST read `docs\/skills-registry\.md` — whole/);
    });
  ...
  describe('🧭 הפרוסה הכללית — `general`  ⟦D-174⟧', () => {
    ...
    /**
     * 🔴 **⟦נכתבה מחדש 15/09 · `D-247` · הוראת רוי⟧ האיסור התהפך, והטענה איתו.**
     *
     * הטענה הישנה נעלה את המשפט «⛔ NEVER. That is QA's alone» — כלומר ש-PM ⛔ אינו מזיז
     * את המוקד לזרימת פיצ׳ר. **רוי הפך את זה במפורש ב-15/09.** ⇒ טענה שממשיכה לנעול את
     * הנוסח הישן הייתה מפילה את הבנייה על הקוד **הנכון**, וזה בדיוק ההפך מהתפקיד שלה.
     *
     * ⛔ **ומה ש⛔ לא התהפך הוא בדיוק מה שהטענה הזאת שומרת עכשיו:** ההפרדה בין
     * **התקדמות** (ספירה, זולה, של כולם) ל**חתימה** (שיפוט, יקרה, של QA בלבד). אם
     * ההפרדה תיעלם מהפרומפט, PM יתחיל לחתום — וזה השער שנשאר.
     */
    it('ⓒ 🔴 הצד השלילי ⟦D-247⟧ — PM מתקדם ✅, ⛔ אבל ⛔ אינו חותם ו⛔ אינו מוציא מהרוטציה', () => {
      const pm = text('PM');
      // ⛔ החתימה — הגדר שנשארה, ו⛔ היא היחידה שנשארה.
      expect(pm, '⛔ PM ⛔ אינו חותם').toMatch(/⛔ You ⛔ still do not seal a workstream/);
      expect(pm, 'ורק חתימה מוציאה מהרוטציה').toMatch(/only a seal takes a department out of the rotation/i);
      // ⛔ «התקדמתי» ⛔ אינו «גמרתי» — בלי המשפט הזה «להתקדם» נקרא כ«לנטוש».
      expect(pm, '⛔ התקדמות ⛔ אינה נטישה').toMatch(/advancing is ⛔ not abandoning/i);
      // 🔴 ופרוסה אחת לטיק — הכלל היחיד שרוי השאיר קשיח.
      expect(pm, 'פרוסה אחת לטיק').toMatch(/ONE SLICE PER TICK/);
      // ⛔ והנוסח הישן ⛔ חייב להיעלם, אחרת שני הכללים חיים בקובץ אחד.
      expect(pm, "⛔ הנוסח הישן ⛔ ירד").not.toMatch(/⛔ NEVER\. That is QA's alone/);
    });

    /**
     * 🔴 **`D-247` — ההתקדמות עצמה, בשלושת הקבצים.** ⛔ בלי הטענה הזאת אפשר למחוק את
     * כל הנוהל מ-DEV ומ-PM והבנייה תישאר ירוקה, כי ⛔ אף בדיקה אחרת ⛔ אינה מזכירה אותו.
     */
    it('ⓔ ‏D-247 — DEV ו-PM מתקדמים כשהספירה אפס, ו⛔ פרוסה אחת לטיק', () => {
      for (const a of ['DEV', 'PM'] as const) {
        expect(text(a), `${a}: ההכרעה`).toContain('D-247');
        expect(text(a), `${a}: הנוהל חי ב-RULES, ⛔ ולא משוכפל`).toMatch(/RULES § 0\.23 ז׳/);
        expect(text(a), `${a}: פרוסה אחת לטיק`).toMatch(/ONE SLICE PER TICK/);
      }
      // ⛔ ו-QA חייב לדעת שהמוקד יכול לזוז בלעדיו — אחרת הוא יפתח ממצא על התנהגות תקינה.
      const qa = text('QA');
      expect(qa, 'QA: יודע שהוא ⛔ לא היחיד').toMatch(/⛔ NO LONGER THE ONLY ONE WHO MOVES IT/);
      expect(qa, 'QA: ⛔ ואינו פותח ממצא על מוקד שזז').toMatch(/that is ⛔ not a defect/i);
      // 🔴 ומה שנשאר שלו — ⛔ בלי זה השחרור בלע את החתימה.
      expect(qa, 'QA: החתימה נשארה שלו').toMatch(/⛔ ONLY A SEAL TAKES A DEPARTMENT OUT OF THE ROTATION/);
    });
...
describe('docs/agents/*.md — שער ה-verify, סוף הנסיגה השקטה, וזמני המסלולים', () => {
   ...
   */
  /**
   * 🌐 **`Kernel` — המחבר שהפך את בדיקת העשן מ«⛔ לא נמדד» לנמדדת.** ⟦NEW 14/09 · רוי⟧
   * 🔬 **הרקע נמדד, ⛔ ולא שוער:** הסביבה של הסשן הזה ⛔ אינה מגיעה לאתר החי
   * (`F-200` — 15 מדידות של `connect_rejected`), ⛔ **אבל היא משתנה בין סביבות**
   * (‏`C-0540` של QA כן הגיע ב-`curl`). ⇒ שער חובה שתוצאתו תלויה בסביבה ⛔ אינו שער.
   * ‏`Kernel` מריץ Chromium בענן ⇒ הוא מוציא את ההגעה מהמשוואה.
   * ⛔ **ושתי הטענות שבלעדיהן זו המלצה ו⛔ לא שער:** ① הפרומפט נוקב ב-`Kernel`;
   * ② הוא מחייב **מחיקת הסשן**, אחרת דפדפן ענן נשאר פתוח וחיובי.
   */
  /**
   * 🧭 **`F-252` — `STEP 5.8`: המוקד מוכרע, או שנכתב למה ⛔ לא.**
   *
   * 🔬 **הכשל שנמדד:** «AND SET THE FOCUS» היה `###` קבור בתוך `STEP 5`, ו⛔ **שום שורה
   * ב-`QA.md` ⛔ לא ציוותה לספור** את ⬜ של `ACTIVE_WORKSTREAM`. שתי האזעקות שהיו אמורות
   * להעלות את השאלה היו עיוורות למוקד חוצה-מערכת ⇒ המוקד ⛔ לא זז 9 טיקי DEV.
   *
   ...
   */
  /**
   * 🧩 **התחילית `anthropic-skills:` — הסקילים ⛔ לא היו חסרים, השם היה שגוי.**
   *
   * 🔬 **נמדד בסשן התפעול 14/09:** הרשימה החיה נושאת את עשרת «סקילי הסשן» שהאינדקס
   * סימן כבלתי-נגישים מאז 09/09 — כל אחד מהם תחת `anthropic-skills:<שם>`. ⇒ ההוראה
   * `/find-skill` בארבעת הפרומפטים נכשלת **על השם**, ⛔ ולא על זמינות.
   *
   * ⛔ **וההסתייגות נבדקת יחד עם העובדה:** ⛔ אין דרך למדוד מהריפו אם ה-Routine נושאת
   * את התוסף — ⇒ הפרומפט חייב לבקש **דיווח**, אחרת נחליף השערה אחת באחרת.
   */
  /**
   * 🔴 **⟦נכתבה מחדש 15/09 · `C-0621` · הוראת רוי⟧ הטענה הזאת נעלה תשובה שהתיישנה.**
   *
   * ‏14/09 היא נעלה «השם הוא `anthropic-skills:find-skill` ⛔ ולא `/find-skill`» + בקשה
   * לדיווח, כי אז ⛔ עוד ⛔ לא ידענו אם ה-Routine נושאת את התוסף. **עכשיו ידוע ונמדד:
   * ‏`account_skills: []` בשש המשימות ⇒ ⛔ אף אחד מהם ⛔ אינו נטען, בשום שם.** ⇒ טענה
   * שממשיכה לדרוש «נסה בשם המלא ודווח» שולחת סוכן לרדוף אחרי כלי ש⛔ אינו קיים,
   * ומבזבזת טיק על ראיה שכבר נאספה.
   *
   * ⇒ **מה שנועל עכשיו הוא הפתרון, ⛔ ולא השאלה:** הסקילים הם **קבצים בריפו**.
   */
  it('ארבעת הפרומפטים שולחים לקובץ בריפו, ⛔ ולא לכלי ש⛔ אינו נטען (15/09)', () => {
    for (const a of ['DEV', 'PM', 'QA', 'CONTENT'] as const) {
      const body = text(a);
      expect(body, `${a}: הנתיב בריפו`).toContain('skills/anthropic/<name>/SKILL.md');
      // ⛔ העובדה שמאחורי הנתיב — בלעדיה «למה» נעלם והשורה תיקרא כהעדפה.
      expect(body, `${a}: העובדה הנמדדת`).toMatch(/account_skills: \[\]/);
      /**
       * 🔴 ⟦התהפך 17/09 · `C-0692`⟧ **הטענה הזאת נעלה במשך תשעה ימים דבר שאינו נכון.**
       * היא דרשה שכל פרומפט יכריז «⛔ אין מה להפעיל, יש מה **לקרוא**», על סמך
       * `account_skills: []` · `enabled_plugins: []` שנמדדו נכון בשש המשימות. **המדידה
       * הייתה נכונה, המסקנה ⛔ לא:** שני השדות מכבים סקילים של **תוסף**; סקיל של
       * **פרויקט** מתגלה מ-`.claude/skills/` ו⛔ אינו תלוי בהם. 🔬 **נמדד 17/09 בסשן ענן
       * חי ששכפל את הענף: 11 סקילים זמינים להפעלה.** ⇒ הטענה הפוכה עכשיו — הפרומפט
       * חייב לשלוח את הסוכן ל-`Skill(...)`, **ולשמור את קריאת-הקובץ כנפילה לאחור**,
       * כי נתיב יחיד בלי נפילה לאחור הוא בדיוק מה ש-`F-189` היה.
       */
      expect(body, `${a}: ⇒ Skill(...)`).toMatch(/Skill\(<name>\)/);
      expect(body, `${a}: ⇒ הנפילה לאחור נשמרת`).toMatch(/skills\/anthropic\/<name>\/SKILL\.md/);
    }
    // 🔴 והקבצים באמת שם — ⛔ טענה על נתיב בלי הקובץ היא הבטחה, ⛔ לא שער.
    for (const k of ['animate', 'apple-design', 'emil-design-eng', 'review-animations', 'find-skill']) {
      expect(
        existsSync(`skills/anthropic/${k}/SKILL.md`),
        `⛔ skills/anthropic/${k}/SKILL.md ⛔ אינו קיים — הפרומפט מפנה לקובץ שאינו שם`,
      ).toBe(true);
    }
  });
   ...
   */
  /**
   * 🔴 **⟦NEW 16/09 · `C-0650` · `F-268`⟧ קובץ סוכן ⛔ אינו רשאי להצהיר לוח זמנים שאינו הלוח החי.**
   *
   * 🔬 **נמדד, ⛔ ולא שוער:** `PROMOTER.md` הצהיר `5 0,12 * * *` בעוד ה-Routine החי הוא
   * `45 7,13` — **שני דורות של לוח אחורה** — והמסדרון כולו נבנה על «DEV יורה כל שעתיים ב-`:30`»,
   * שגם הוא כבר ⛔ אינו נכון. ⇒ הסוכן קרא שהפער הוא 95 דקות, הסיק שנעילה שהוא פוגש היא **תקועה**,
   * ובפועל הפער הוא **40 דקות** וההחזקה החציונית של DEV היא 26. ⇒ ב-16/09 הוא רץ 12.5 דקות והשאיר
   * **אפס קומיטים** בזמן ש-136 קומיטים ירוקים חיכו ב-`dev` וכל חמשת תנאי השער היו ירוקים.
   *
   * ⛔ **וזו ההישנות השנייה באותו יום:** `C-0625` הזיז cron ועדכן `roster.json` ו-`LOOP-ARCHITECTURE`
   * ⛔ אבל ⛔ לא את הכותרת של `PROMOTER.md`. ⇒ הטענה הזאת עושה את הסחיפה **בלתי-אפשרית**, ⛔ ולא
   * «משהו שצריך לזכור»: המקור הוא `roster.json`, והקבצים נמדדים מולו.
   */
  it('⛔ אף קובץ סוכן ⛔ אינו מצהיר cron שאינו ב-`roster.json` (F-268)', () => {
    const roster = JSON.parse(readFileSync('docs/agents/roster.json', 'utf8')) as {
      agents: { name: string; cronUTC: string }[];
    };
    // ⛔ כל שדות ה-cron החיים, מפורקים לביטויים בודדים (QA נושא שניים בשדה אחד).
    const live = new Set(
      roster.agents.flatMap((a) =>
        a.cronUTC.split('·').map((part) => part.replace(/\(.*?\)/g, '').trim()).filter(Boolean),
      ),
    );
    expect(live.size, '⛔ הרוסטר חייב להצהיר לפחות חמישה ביטויי cron').toBeGreaterThanOrEqual(5);

    const offenders: string[] = [];
    for (const who of ['DEV', 'PM', 'QA', 'CONTENT', 'PROMOTER'] as const) {
      // ⛔ כל מחרוזת שנראית כביטוי cron בתוך גרשיים אחוריים, ⛔ ולא פרוזה על שעות.
      for (const m of text(who).matchAll(/`(\d[\d,\s]*(?:\*\/\d+|\*)[\s\S]{0,12}?\*\s\*)`/g)) {
        const expr = (m[1] ?? '').replace(/\s+/g, ' ').trim();
        if (!live.has(expr)) offenders.push(`${who}.md: \`${expr}\``);
      }
    }
    // ⛔ ההודעה **היא** רשימת התיקון: כל שורה היא קובץ שמשקר לסוכן שקורא אותו בכל טיק.
    expect(offenders, 'cron בקובץ סוכן שאינו ב-roster.json').toEqual([]);
  });

  it('⛔ QA ⛔ אינו שואל את השאלה שהוכחה כבלתי-ניתנת-להתקיים (F-261)', () => {
    const qa = text('QA');
    // ⛔ **הנוסח שנמדד כשגוי ⛔ אינו חוזר כמבחן ההכרעה.** ‏`F-252` שאל «האם `general`
    //    **עצמה** ⬜=0», ו-`F-261` מדד שהתשובה ⛔ לעולם ⛔ אינה «כן» — PM פותח לתוכה.
    expect(qa, '⛔ ⛔ לא מבחן ה-⬜ של המוקד עצמו').not.toMatch(
      /= 0 ⬜ {2,}⇒ 🔴 DECIDE IN THIS TICK\. Move it/,
    );
    // …והמבחן החדש נוקב **ברצף**, ⛔ ולא בחדר ההמתנה.
    expect(qa, 'המבחן הוא הרצף').toMatch(/AND the sequence holds work/);
    expect(qa, 'וחדר המתנה ⛔ אינו מחלקה').toMatch(/waiting room is ⛔ not a\s+department/i);
    expect(qa, 'נוקב בממצא').toContain('F-261');
    // ⛔ **ושלושת הקוראים אומרים את אותו דבר** — זה בדיוק מה שנשבר: `C-0625` תיקן
    //    שניים מתוך שלושה, ו-`QA.md` נשאר מאחור עם ההוראה ההפוכה.
    for (const who of ['DEV', 'PM', 'QA'] as const) {
      expect(text(who), `${who} — מפנה לנוהל אחד`).toMatch(/§ 0\.23 ז׳/);
    }
  });
  ...
  it('QA מכריע על המוקד ב-STEP 5.8, וסירוב להזיז חייב שורה (F-252)', () => {
    ...
    expect(qa, 'הממצא נקוב בשמו').toMatch(/`F-252`/);
    expect(qa, 'ציווי לספור, ⛔ לא לחוש').toMatch(/COUNT IT, ⛔ DO NOT SENSE IT/);
    expect(qa, '⬜ של הזרימה עצמה, ⛔ לא של הבריכה').toMatch(
      /the ⬜ count of `ACTIVE_WORKSTREAM` \*\*itself\*\*/,
    );
    // ⛔ והצד שמונע את החזרת הכשל: «הבריכה מלאה» ⛔ אינה תשובה.
    expect(qa, '«הבריכה מחזיקה עבודה» ⛔ אינה תשובה').toMatch(
      /«the pool still has work» is ⛔ NOT an answer/,
    );
    expect(qa, 'אי-הזזה חייבת שורה ב-00-control').toMatch(/המוקד נשאר/);
  });

  it('QA ו-PROMOTER מושכים את בדיקת העשן דרך Kernel, ו⛔ סוגרים את הסשן', () => {
    for (const a of ['QA', 'PROMOTER'] as const) {
      const body = text(a);
      expect(body, `${a}: המחבר נקוב בשמו`).toMatch(/`Kernel`/);
      expect(body, `${a}: יוצר סשן דפדפן`).toMatch(/manage_browsers create/);
      expect(body, `${a}: מריץ את הקריאה`).toMatch(/execute_playwright_code/);
      expect(body, `${a}: ⛔ סוגר את הסשן — דפדפן ענן דולף הוא חיוב`).toMatch(
        /manage_browsers delete/,
      );
      // ⛔ הנפילה לאחור ⛔ אינה נמחקת: אם המחבר ⛔ אינו זמין בריצה, «⛔ לא נמדד» עדיין
      // הניסוח היחיד המותר — ⛔ ולעולם לא «עבר».
      expect(body, `${a}: הנפילה לאחור שרדה`).toContain('⛔ לא נמדד');
    }
  });

  /**
   * 🔬 **דגימה אחת של `/api/health` ⛔ אינה ראיה — נמדד פעמיים באותו יום.**
   * ‏`C-0607` רשם `ok:false` ב-4 מתוך 6 משיכות ב-13:32Z; אותו נתיב החזיר
   * `ok:true` ב-6 מתוך 6 ב-18:29Z. ⇒ שני הפרומפטים חייבים לדרוש **דגימה מרובה**.
   */
  it('⛔ אף פרומפט ⛔ אינו מכריע על בדיקת העשן מדגימה אחת', () => {
    for (const a of ['QA', 'PROMOTER'] as const) {
      expect(text(a), `${a}: נדרשות ≥3 משיכות`).toMatch(/3 times|≥3|least 3/);
    }
  });
...
describe('docs/agents/PROMOTER.md — גבול הסמכות ושער הקידום', () => {
   ...
   * 🔬 **נמדד חי C-0599:** ops נעל ב-08:39:20Z, השער רץ 86 שניות, ‏PM נעל את **אותו**
   * מזהה ב-08:40:00Z, והדחיפה של ops נדחתה ב-08:41:05Z. ⇒ חלון המרוץ הוא כל ריצת השער.
   * ⇒ **ארבעת הדוחפים חייבים לדחוף את קומיט הנעילה לבדו**, אחרת הוא נגרר למסלול הארוך.
   */
  it('ארבעת הדוחפים דוחפים את קומיט הנעילה לבדו לפני העבודה (F-251)', () => {
    for (const a of ['DEV', 'PM', 'QA', 'CONTENT'] as const) {
      const body = text(a);
      expect(body, `${a}: הממצא נקוב בשמו`).toMatch(/`F-251`/);
      expect(body, `${a}: הנעילה לבדה`).toMatch(/commit ⛔ ONLY plan\/00-control\.md with the lock/);
      expect(body, `${a}: ודוחפים אותה לבדה`).toMatch(/push it, and ⛔ nothing with it/);
      // ⛔ וההתקנה קודמת לנעילה — הכלל של `F-195` גובר על הכלל של `F-251`:
      // מרוץ אבוד עולה טיק, קומיט ראשון ללא שער עולה את השער עצמו.
      expect(body, `${a}: hooks:install לפני הנעילה`).toMatch(
        /npm run hooks:install\s+⇐ FIRST, always/,
      );
      // ⛔ והצד השני של הכלל: מפסיד מרוץ מחשב מזהה מחדש באותו טיק, ⛔ ואינו נוטש
      // את העבודה על ענף claude/* — זה בדיוק F-248.
      expect(body, `${a}: מפסיד מרוץ ממשיך באותו טיק`).toMatch(
        /recompute the cycle id and start again in this same tick/,
      );
      expect(body, `${a}: ⛔ ולא דוחף ל-claude/*`).toMatch(/`F-248`/);
    }
  });
...
/**
 * 🧭 ⟦11/09 · Roy's instruction 1.4⟧ **מרחב ההכרעה ⇒ העברה מפורשת, ⛔ ולא דילוג בשקט.**
 *
 * ⛔ **ולמה זו טענה אחת ו⛔ לא מנגנון חדש.** נמדד 11/09 לפני הכתיבה: המנגנון **כבר קיים
 * ואחיד** — `§ 0.20` (טבלת הניתוב) מצוטטת בחמשת הקבצים, `§ 0.31` (מרחבי ההכרעה של PM
 * ו-QA) מצוטטת בשניים ו**נבדקת** בטענות מהותיות, ו**חמשת הקבצים נושאים את אותה שורה
 * בדיוק** בבלוק «מה ⛔ אינו טיק לגיטימי»:
 *     ⛔ NOT "I am waiting for <agent>" ⇒ route it (`RULES § 0.20`) and take the next thing
 * ⇒ ⛔ **אין מה לבנות.** מה שחסר היה **טענה**: ⛔ שום דבר ⛔ לא מנע מהשורה הזאת להיעלם
 * מקובץ אחד בעריכה, ואז הסוכן ההוא היה חוזר לדלג בשקט ⛔ בלי שאף בדיקה תאדים.
 * ⇒ הטענה הזאת מקבעת **מציאות שנמדדה**, ⛔ ואינה כופה נוסח חדש על אף סוכן.
 */
describe('🧭 חובת ההעברה — חמשת הקבצים, ⛔ ולא שלושה מהם', () => {
  for (const agent of ALL_PROMPTS) {
    it(`${agent}: «ממתין לסוכן» ⛔ אינו טיק לגיטימי — מנתב ולוקח את הבא`, () => {
      const body = text(agent);
      // ⛔ הפנייה לטבלת הניתוב, ⛔ ולא רק אזכור של הסעיף.
      expect(body, '⛔ טבלת הניתוב ⛔ אינה מצוטטת').toContain('RULES § 0.20');
      // 🔴 והחובה עצמה: «ממתין» ⛔ אינו מצב — הוא ניתוב שלא נעשה.
      expect(
        /I am waiting for <agent>[^\n]*route it/.test(body),
        `${agent}: ⛔ השורה «⛔ NOT "I am waiting for <agent>" ⇒ route it» חסרה ⇒ ` +
          'הסוכן יכול לדלג בשקט על מה שמחוץ למרחבו',
      ).toBe(true);
      // ⛔ ורוי ⛔ אינו חריג: גם המתנה לו ⛔ אינה עצירה, היא שורה חתומה והמשך.
      expect(
        /I am waiting for Roy[^\n]*(never|⛔)/.test(body),
        `${agent}: ⛔ «ממתין לרוי» ⛔ אינו מסויג`,
      ).toBe(true);
    });
  }
});
