...
describe('docs/agents/*.md — שער ה-verify, סוף הנסיגה השקטה, וזמני המסלולים', () => {
   ...
   */
  it('ארבעת הפרומפטים נוקבים בשם המלא של הסקיל, ⛔ ולא ב-/find-skill (14/09)', () => {
    for (const a of ['DEV', 'PM', 'QA', 'CONTENT'] as const) {
      const body = text(a);
      expect(body, `${a}: התחילית המלאה`).toContain('anthropic-skills:find-skill');
      expect(body, `${a}: ⛔ והשם הישן ⛔ אינו עומד לבדו כהוראה`).toMatch(
        /השם הוא `anthropic-skills:find-skill`, ⛔ ולא/,
      );
      expect(body, `${a}: ומבקש ראיה מהטיק עצמו`).toMatch(/ואמור בדוח מה קרה/);
    }
  });
