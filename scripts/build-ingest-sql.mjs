...
const { spotCheckPlan } = await import('../lib/core/spotCheck.ts');
...
// --- 4. ONE SPOT-CHECK PLAN PER BATCH ----------------------------------------
//
// ⛔ **This used to plan over `passing` — the WHOLE bank — and that was the deviation,
// ⛔ not the rule.** `R-014` (plan/20-alerts.md) says, word for word:
//     «דגימת בקרה אנושית **לכל אצווה** — לפי ISO 2859-1 AQL 1.0…»
// «לכל אצווה» is *per batch*. The code sampled the union of every batch ever written, so
// the lot grew monotonically and hit `DOCUMENTED_MAX` (1,200) — measured 08/09: the bank
// stood at exactly 1,200 rows across 25 files, and `npm run build:ingest` threw
// `RangeError` on the next row anyone added. CONTENT had produced zero content for two
// days because of it (`F-194`, `K-005` priority b), and CONTENT ⛔ may not touch code.
//
// ⇒ **This is the code conforming to R-014, ⛔ not R-014 being relaxed.** And it is
// STRICTER, ⛔ not looser: batches measure 3–66 rows (median 50), all far below
// `DOCUMENTED_MIN` (501), so every one takes `spotCheck.ts`'s `basis: 'full'` branch —
// **100% inspection**, against 80-of-1,200 before. It also has ⛔ no ceiling to grow into.
//
// 🔴 **⟦10/09⟧ `R-014` הוחלפה: ⛔ אין עוד דגימת בקרה אנושית על שום סוג תוכן.**
// ⇒ התוכנית לכל אצווה עדיין מחושבת ונרשמת בכותרת, כי היא **תיעוד של הבסיס** —
// כמה שורות באצווה ומה היה הבסיס לבדיקה — ⛔ אבל ⛔ אין עוד **בחירת שורות שאדם
// אמור לעבור עליהן**, כי ⛔ אין עוד אדם כזה בתהליך.
// ⛔ **`lib/core/spotCheck.ts` עצמו ⛔ אינו נוגע.** השורה היחידה של ISO 2859-1 ו«⛔ אל
// תשלים את הטבלה» עומדות — הנימוק שלהן הוא על ראיות ש⛔ אין לנו, ו⛔ זה ⛔ לא השתנה.
// ⇒ מה שנושא את האות עכשיו: `translation_confidence` (מילים) ו-`grammar_confidence`
// (שאלות ותרגילים), שניהם **מדגם שהכותב מצהיר**, ⛔ ולא תור אנושי.
for (const batch of batches) {
  batch.plan = spotCheckPlan(batch.passing.length);
}
...
const lines = [
  ...
  '--',
  `-- ${batches.length} batches · ${passing.length} rows`,
...
for (const batch of batches) {
  ...
  lines.push(
    `-- lot ${batch.plan.lotSize} · inspect ${batch.plan.inspect} · accept up to ${batch.plan.acceptUpTo} · basis: ${batch.plan.basis}`,
  );
  lines.push(`-- ${'-'.repeat(74)}`);
...
// ⛔ Per batch, ⛔ not one number for the bank — a single line here is exactly what hid
// the fact that the lot was the union of every batch ever written (F-194).
const full = batches.filter((b) => b.plan.basis === 'full').length;
// ⛔ ⟦10/09⟧ ⛔ אין עוד «שורות שנבחרו לבדיקה אנושית» — `R-014` הוחלפה. מה שנשאר
// מדווח הוא **הבסיס לכל אצווה**, שהוא תיעוד, ⛔ ולא תור.
console.log(
  `lots (R-014, replaced 10/09): ${batches.length} · ${full}/${batches.length} at basis full · ⛔ no human queue`,
);
