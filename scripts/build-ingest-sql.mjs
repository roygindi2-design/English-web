...
//
// ⛔ **`lib/core/spotCheck.ts` is ⛔ NOT touched.** Its single ISO row and its
// «⛔ Do not "complete the table"» stand — that reasoning is about evidence we ⛔ do not
// have, and ⛔ nothing here changes it.
for (const batch of batches) {
  batch.plan = spotCheckPlan(batch.passing.length);
  batch.inspected = selectForSpotCheck(batch.passing, batch.plan);
}
const inspected = batches.flatMap((b) => b.inspected);
...
const lines = [
  ...
  '--',
  `-- ${batches.length} batches · ${passing.length} rows · spot-checked PER BATCH (R-014), ${inspected.length} rows selected`,
  '-- Each batch header below carries its own lot · inspect · accept · basis.',
  '--',
...
console.log(
  `spot-check per batch (R-014): ${batches.length} lots · ${inspected.length} rows selected · ${full}/${batches.length} at basis full`,
);
