...
describe('scripts/measure-continuations.mjs', () => {
   ...
   * ⛔ לא נגעתי בה) · `order2` שחדל לצמצם כלל · ושניהם מתכנסים זה לזה.
   * 🗑️ ⟦25/09 · `D-294`⟧ רצפת ה-10% על `order2` הוסרה — ראה בגוף הטענה.
   */
  it('מודל סדר 2 מצמצם, ⛔ וסדר 1 לא — היחס הוא הראיה, ⛔ לא ההפרש', () => {
    const [order1, order2] = reductions();
    expect(order1).toBeDefined();
    expect(order2).toBeDefined();
    // 🗑️ ⟦25/09 · `D-294` · הכרעת רוי, `for-roy` 148⟧ **רצפת ⓐ (`order2 >= 10`) הוסרה.**
    // 🔬 אצוות `C-0830` (234 שורות word-only) הורידה את `order2` ל-**9.0%** בעוד `order1`
    // נשאר **0.1%** ⇒ היחס **פי 90** — המסקנה ⛔ לא זזה, רק הקורפוס גדל. ⇒ אותה מחלקה בדיוק
    // כמו `F-286`: סף מוחלט על מדד שמתכווץ עם הקורפוס. ⇒ המסקנה נשמרת בשתי טענות חסינות-גודל:
    // `order1 < 10` (למעלה, ⛔ לא נגעתי) ו-ⓑ היחס. ‏`order2` חייב עדיין להיות **חיובי**.
    expect(order2 as number, 'סדר 2 ⛔ אינו מצמצם כלל').toBeGreaterThan(0);
