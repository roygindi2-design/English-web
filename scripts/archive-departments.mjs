 ...
 *
 * ⇒ **שלושה תנאים, ו⛔ אחד ⛔ אינו מספיק:**
 *   ① תא היעדים נושא `חתומה`  — המילה שמסמנת מחלקה שנסגרה
 *   ② ⛔ והמילה ⛔ אינה נשללת — «אינה חתומה» · «לא חתומה» ⇒ ⛔ לא נסגרה ⟦T-336⟧
...
/**
 * 🔴 ⟦T-336 · 14/09⟧ **שלילת החתימה — ⛔ המילה `חתומה` לבדה ⛔ אינה עדות.**
 *
 * 🔬 **נמדד C-0589 (PM) חי בקלון, ⛔ ולא שוער:** קיצור שורת `arena` לתקרת בדיקה 19
 * הוריד ממנה את המילה `חסומות`. התא שנשאר — «⛔ אין ⬜. ⛔ אינה חתומה — ⓑⓒ ב-env» —
 * ⛔ אינו נושא ⛔ אף אחד מ-`OPEN_MARKS` (‏`ⓑ`/`ⓒ` הן אותיות מוקפות, ⛔ ולא ספרות),
 * ⇒ ‏`/חתומה/` התאים בתוך **`אינה חתומה`**, המחלקה אורכבה, ו-`npm run verify` נפל.
 * ⇒ **מחלקה שמצהירה במפורש ש⛔ לא נסגרה מחקה את עצמה.**
 *
 * ⛔ **וההגנה שהייתה כאן עד היום היא צירוף מקרים, ⛔ ולא כלל:** שלוש המחלקות
 * הלא-חתומות נשאו `חסומ`/`T-NNN` **במקרה** ⇒ **עריכת ניסוח מוחקת חסם חי**, וזה
 * בדיוק מה שההערה בראש הקובץ (`:12-17`) אומרת שאסור שיקרה.
 * ⇒ ⛔ הכלל ⛔ אינו «חפש סימן פתוח אחר»: הוא **קרא את השלילה עצמה**.
 */
const NOT_SEALED = /(?:אינה|איננה|לא|טרם)\s+(?:[⛔\s]+)?חתומה/;

export function isSealedRow(goalsCell) {
  if (!/חתומה/.test(goalsCell)) return false;
  if (NOT_SEALED.test(goalsCell)) return false;
