...
for (let i = 0; i < ORDERED.length; i += 1) {
  ...
  if (spent > 0) {
    ...
    if (tally(byStream.get(here) ?? []).open === 0) {
      ...
      /**
       * 🔴 **⟦`C-0789` · `T-419`⟧ ורשימת המועמדים נפלטת גם כאן, ⛔ ולא רק בחדר ההמתנה.**
       * 🔬 **נמדד ב-`C-0698`:** `studies` הגיעה ⬜=0, `§ 0.23 ז׳` ④ אמר «הרשימה כבר מחושבת ⇒
       * ⛔ אל תגזור בעצמך», ו-`grep 'מועמדים'` על הפלט החזיר **0** — כי הבלוק למטה נפלט ⛔ רק
       * כשהמוקד חוצה-מערכת. ⇒ ההוראה ⛔ לא הייתה בת-ביצוע, והסוכן גזר ביד.
       * ⇒ **אותה רשימה, ⛔ נימוק אחר**: כאן המחלקה הפעילה מוצתה, ושם חדר ההמתנה יושב בזמן
       * שיש עבודה. ⛔ «הבא עם עבודה» לבדו ⛔ אינו מספיק — רק הסוכן יודע אם שורה כשירה **לו**
       * (`msgs`: שורה אחת, של CONTENT).
       */
      const exhaustedCandidates = rotation
        .map((w) => ({ w, open: tally(byStream.get(w) ?? []).open }))
        .filter((c) => c.open > 0);
      const candidateLine = exhaustedCandidates.length === 0
        ? ''
        : `\n  **מועמדים לפי הסדר** (גלגול מ-\`${here}\`, **המחלקה הפעילה שמוצתה**): ` +
          exhaustedCandidates.map((c) => `\`${c.w}\` (${c.open} ⬜)`).join(' · ') +
          `. ⇒ **קח את הראשון שיש בו שורה כשירה לך** (\`§ 0.23 ז׳\` ④), ⛔ ולא בהכרח את הראשון ברשימה, ⛔ ודלג על מחלקה חתומה.`;
      if (here === ACTIVE_WORKSTREAM) {
        flags.push(
          `🔴 **הזרימה הפעילה מוצתה — \`${here}\` (${spent} טיקים) ו⛔ אין בו אף משימה ⬜ פנויה.** ${where}${candidateLine}
` +
