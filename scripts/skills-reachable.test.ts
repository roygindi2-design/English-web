...
/**
 * 🔴 ⛔ **`.claude/skills/` — הנתיב היחיד ש-Claude Code באמת סורק.**
 *
 * 🔬 **נמדד 17/09, ⛔ ולא שוער.** הלופ חי מאז 08/09 לפי המסקנה «⛔ אין מה להפעיל, יש מה
 * **לקרוא**» (`DEV.md` · `PM.md` · `QA.md` · `CONTENT.md`), והיא נשענה על `enabled_plugins: []`
 * שנמדד בכל שש המשימות. **המדידה נכונה — המסקנה ⛔ לא.** `enabled_plugins` מכבה סקילים של
 * **תוספים**; סקיל של **פרויקט** מתגלה מ-`.claude/skills/<name>/SKILL.md` ו⛔ אינו תלוי בו
 * כלל, ותיעוד ה-Routines אומר במפורש שסשן מתוזמן «can … **use skills committed to the
 * cloned repository**». ⇒ 25 קובצי ה-`SKILL.md` שלנו ישבו ב-`skills/` — **נתיב שאינו נסרק**.
 *
 * **המחיר של הנתיב השגוי, מדוד בבתים:** פרוזת הסקילים היא 12,980 ב׳ ב-`DEV.md` (14.3%
 * מהקובץ) ועוד 18,500 ב׳ של רישום שחובה לקרוא במלואו ⇒ **≈13,300 טוקנים בכל טיק**, לפני
 * שנפתח ולו `SKILL.md` אחד.
 *
 * ⇒ הטענה הזאת מחזיקה **אינווריאנטה אחת**: כל ספרייה תחת `.claude/skills/` נושאת `SKILL.md`
 * **קריא**, ושמו המוצהר **שווה לשם הספרייה** — כי שם שאינו תואם הוא סקיל שהסוכן ⛔ אינו
 * יכול לנקוב בשמו, כלומר בדיוק הכישלון שכל הקובץ הזה קיים נגדו.
 *
 * ⛔ **ומה שהיא ⛔ אינה טוענת:** שהגילוי אכן קורה בטיק מתוזמן. זה ⛔ אינו ניתן להוכחה
 * מבדיקת יחידה — הוא הוכח בסשן חי (`C-0692`), וזה ⛔ תנאי לשלב מחיקת הפרוזה.
 */
describe('.claude/skills/ — סקילים שניתן להפעיל, ⛔ ולא רק לקרוא', () => {
  const dirs = execFileSync('git', ['ls-files', '.claude/skills/'], { encoding: 'utf8' })
    .split('\n')
    .filter((f) => f.endsWith('/SKILL.md'));

  it('⛔ אינו ריק — אחרת כל השאר חלול', () => {
    expect(dirs.length, '⛔ אף סקיל תחת .claude/skills/').toBeGreaterThanOrEqual(11);
  });

  it('🔴 כל SKILL.md קריא בפועל — ⛔ וסימלינק שבור ⛔ אינו «קיים»', () => {
    const broken: string[] = [];
    for (const f of dirs) {
      // ⚠️ `existsSync` לבדו ⛔ אינו מספיק: הוא עוקב אחרי הסימלינק ומחזיר false על שבור,
      //    ⛔ אבל ⛔ אינו מבחין בין «אין» ל«יש וריק». קריאה בפועל מבחינה.
      try {
        if (readFileSync(f, 'utf8').trim().length === 0) broken.push(`${f}: ריק`);
      } catch {
        broken.push(`${f}: ⛔ אינו קריא`);
      }
    }
    expect(broken, `⛔ סקיל שאינו נטען: ${broken.join(' · ')}`).toEqual([]);
  });

  it('🔴 השם המוצהר = שם הספרייה — ⛔ אחרת הסוכן ⛔ אינו יכול לנקוב בו', () => {
    const drift: string[] = [];
    for (const f of dirs) {
      const dir = f.split('/').at(-2) as string;
      const declared = (/^name:\s*(.+)$/m.exec(readFileSync(f, 'utf8'))?.[1]?.trim() ?? '')
        // ⛔ מרכאות ⛔ אינן הפרש-שם — אותו לקח בדיוק כמו בטענה שמעל (`C-0621`).
        .replace(/^(['"])(.*)\1$/, '$2');
      if (declared !== dir) drift.push(`${dir} ⇒ «${declared}»`);
    }
    expect(drift, `⛔ שם שאינו תואם: ${drift.join(' · ')}`).toEqual([]);
  });
});
